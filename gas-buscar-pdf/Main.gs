...
function doGet(e) {
  return responseJson({
    status: 'ok',
    version: VERSION,
    capacidades: ['buscar', 'catch-all-reenvios', 'etiquetar-leido', 'auto-archivado', 'mail-resumen', 'ocr-imagenes', 'soft-match', 'auditar', 'confirmar', 'renombrar', 'listar'],
...
function doPost(e) {
  ...
  try {
    ...
    // Acción 'listar': enumera los archivos de la carpeta del período SIN OCR (para conciliar saldos rápido).
    if (body.accion === 'listar') {
      return listarArchivos(body)
    }
 ...
 * Soporta los formatos reales que aparecen en los comprobantes:
 *   - Argentino (coma decimal) CON o SIN separadores de miles: "1.312.600,00" · "1312600,00" · "600,00"
 *     (ARCA expone el texto SIN separadores → "1312600,00"; el regex viejo solo leía el CON separadores
 *      y capturaba "600,00" → rompía el chequeo de monto y dejaba estándar como huérfanas).
 *   - Punto decimal (US/algunos servicios) CON o SIN separadores: "1,234,567.89" · "1234567.89".
 */
function extraerMontosPdf(texto) {
  const montos = []
  let m
  // Coma decimal (es-AR). Integer con grupos de miles por punto, o entero plano. Termina en exactamente 2 decimales.
  const regexAR = /(\d{1,3}(?:\.\d{3})+|\d+),(\d{2})(?!\d)/g
  while ((m = regexAR.exec(texto)) !== null) {
    const num = parseFloat(m[1].replace(/\./g, '') + '.' + m[2])
    if (!isNaN(num) && num > 0) montos.push(num)
  }
  // Punto decimal. Integer con grupos de miles por coma, o entero plano. Termina en exactamente 2 decimales.
  const regexUS = /(\d{1,3}(?:,\d{3})+|\d+)\.(\d{2})(?!\d)/g
  while ((m = regexUS.exec(texto)) !== null) {
    const num = parseFloat(m[1].replace(/,/g, '') + '.' + m[2])
    if (!isNaN(num) && num > 0) montos.push(num)
  }
  return montos
...
// Acción 'listar': enumera los archivos de la carpeta del período (id, nombre, url) SIN OCR.
// 🔒 SOLO LECTURA: no abre, no convierte, no toca nada. Para conciliar saldos rápido (huérfanos vs
// faltantes) sin el costo del OCR. Ignora los logs _AUDIT_ y los no-documentos (xlsx, etc.).
function listarArchivos(body) {
  ...
  for (var i = 0; i < subs.length; i++) {
    ...
    if (!it.hasNext()) return responseJson({ status: 'ok', existe: false, archivos: [] })
    carpeta = it.next()
  }
  const archivos = []
  ...
  while (files.hasNext()) {
    ...
    if (/^_AUDIT_/i.test(nombre)) continue
    if (!/\.(pdf|jpe?g|png|gif|webp|heic|tiff?)$/i.test(nombre)) continue
    archivos.push({ file_id: f.getId(), archivo: nombre, url: f.getUrl() })
  }
  return responseJson({ status: 'ok', existe: true, archivos: archivos })
}
...
function facturaCoincide(texto, textoNum, f) {
  ...
  const numeroOk = texto.indexOf(pv5 + '-' + nro8) >= 0                             // 00002-00002021
  ...
  // CHEQUEO DE MONTO (clave para evitar falsos positivos: CUIT compartido + un número que aparece por
  // azar en otro comprobante). Si la factura trae imp_total, exigir que ese monto (en valor ABSOLUTO,
  // las NC vienen negativas) aparezca en el PDF con tolerancia $1. Si el OCR no leyó un monto que
  // coincida → NO es match (queda huérfano para asignar a mano). Si no viene imp_total, no se exige.
  if (f.imp_total !== undefined && f.imp_total !== null && f.imp_total !== '') {
    const esperado = Math.abs(parseFloat(f.imp_total))
    if (esperado > 0) {
      const montos = extraerMontosPdf(texto)
      const hayMonto = montos.some(function (m) { return Math.abs(Math.abs(m) - esperado) <= 1.0 })
      if (!hayMonto) return false
    }
  }
  return true
}
