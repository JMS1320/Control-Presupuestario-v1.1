...
Project Settings (engranaje) → Script Properties → **Add script property**:
- Key: `GAS_AUTH_TOKEN`
...
- **Carpeta `_Revisar`**: cuando el match es dudoso (monto fuera de tolerancia,
  múltiples PDFs candidatos), GAS crea subcarpeta `_Revisar` dentro de la
  carpeta de la empresa y deja ahí el archivo. Vos podés ir periódicamente y
