...
- **E2 en curso** (pend. test, commits `2fe8c89`/`48019a0`/`0cd2617`): ✅ **E2.1 Subtotales** (`lib/pagos/subtotales.ts`) · ✅ **E2.3 Comprobante PDF** (`lib/pagos/pdf-detalle-pago.ts`, copia verbatim del Modal) · ✅ **E2.4 Export lote Galicia** (reusa el `ModalExportarLote` existente — CBU/mail/proveedores adentro). Botones en Modo Pagos del Cash Flow. Falta E2.2 Agrupar. ⚠️ **A testear con cuidado** (no pude testear): armado de items del export (grupos `facturas_agrupadas>1`→`tipo:grupo`, schema desde `origen_tabla`, empresa='MSA' hardcode) y que el PDF salga idéntico al del Modal.
