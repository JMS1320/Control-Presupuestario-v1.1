...
| A-BUG-111 | 🔴 | **ALTA** | 🐞 **El parser del romaneo devuelve CERO en el navegador** — nota «Romaneo Error» del 06/09. Cabecera vacía, `0 líneas · 0 medias`, todos los controles en `leído 0`, **y sin ningún error**. ⚠️ **El mismo PDF se lee perfecto en Node con el mismo código** (18.750.900, 9 líneas), y la carga sí se leyó — el bloque de desbaste calculó bien. Así que el corte está en el navegador. ✅ **Agregado un diagnóstico que lo va a decir**: cuántos streams vio, cuántos descomprimió, cuántos tenían texto, y si el navegador tiene `DecompressionStream`. Más reintento recortando la cola del stream. ⏳ **Falta volver a correrlo y leer ese aviso** | Nota del usuario `@ganaderia` |
...
| A-BUG-113 | 🟡 | Media | **La cinta de diagnóstico no capturó el error del romaneo.** La nota trajo sólo un warning de accesibilidad de Radix (`Missing Description for DialogContent`), que es ruido. El fallo real era **silencioso** — no hubo excepción — así que no había nada que capturar. 🔑 La lección no es ampliar la whitelist: es que **un código que devuelve vacío sin quejarse es invisible para cualquier cinta**. Se arregla en el origen (`A-BUG-111`), no en el capturador. ⏳ Aparte: filtrar ese warning de Radix, que ensucia todas las notas | `@notas` |
