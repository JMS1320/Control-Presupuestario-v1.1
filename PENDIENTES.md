...
- **B-FEAT-VENTAS-COBROS — Importar ventas y registrar cobros (incl. retenciones que no entran a banco)** — *analizado 2026-07-01, falta definir 4 puntos*:
  ...
  - **A DEFINIR antes de construir**: (i) retenciones — ¿sumar `ret_ganancias`? ¿cuenta contable fija por tipo o elegible por cobro?; (ii) `estado` de cobro (importado→a cobrar→cobrado); (iii) ¿el cobro aplica igual a granos y factura, o granos ya usan `fecha_acreditacion`?; (iv) ¿un cobro puede saldar varias facturas (tipo grupo de pago) o 1:1?
