...
#### 🔴 URGENTE — Chequear quincenas SICORE mal asignadas por bug de zona horaria (2026-07-16)
- **Bug (ya corregido en código, commit pendiente):** `generarQuincenaSicore` hacía `new Date("YYYY-MM-DD")` (UTC medianoche) y leía `.getDate()` en local → en Argentina (UTC−3) **corría 1 día para atrás**. Solo cambia la quincena cuando la fecha cae el **día 16** (→ quedaba 1ra en vez de 2da) o el **día 1** (→ quedaba en la 2da del mes ANTERIOR). Estaba en las 2 copias (lib `lib/sicore/quincena.ts` + local del Modal `vista-facturas-arca` ~l.3255). Fix: parsear el string directo, sin `Date`.
- **Acción:** query `msa.sicore_retenciones` (no anulados) donde `EXTRACT(DAY FROM fecha_pago) IN (1,16)` → recalcular la quincena correcta y comparar con `quincena` guardada. Ojo también con `comprobantes_arca.sicore` (el estampado). Revisar con el usuario antes de corregir dato (regla: no tocar datos sin preguntar). Prioridad: antes de declarar quincenas que incluyan pagos del 1 o 16.
