...
- **B-FEAT-VENTAS-COBROS — Importar ventas y registrar cobros (incl. retenciones que no entran a banco)** — *analizado 2026-07-01, falta definir 4 puntos*:
  ...
  - **⏳ Fase 2 (NO hecho)**: (a) caso complejo **cuenta corriente / compensación** (AFA: venta granos ↔ compra insumos, cobrar saldo neto); (b) **control Venta↔Factura↔Cobro** (las 3 partes consistentes: la venta se pacta → se factura → se cobra; controlar que la factura cuadre con la venta y el cobro con la factura). Hoy hecho el eslabón **Factura↔Cobro**; falta **Venta↔Factura** (usar `ventas_comprobantes` N:N + comparar totales/ton/precio).
