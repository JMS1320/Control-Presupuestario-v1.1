...
- **B-FEAT-VENTAS-COBROS — Importar ventas y registrar cobros (incl. retenciones que no entran a banco)**: extender el módulo de ventas (`msa.ventas` / `liquidaciones_venta` — ver memoria [[ventas-msa]]) con: (1) **importar** ventas (flujo a definir, similar a ARCA compras); (2) **registrar cobros** de cada venta/liquidación, contemplando que parte del cobro son **retenciones sufridas que NO entran al banco** (Ganancias / IVA / IIBB que el cliente retiene): el cobro neto bancario + las retenciones deben sumar el total, y las retenciones se registran aparte (no como ingreso de caja). A detallar el flujo con el usuario antes de implementar.
