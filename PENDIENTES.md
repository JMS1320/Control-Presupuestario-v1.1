...
#### 🧾 SICORE — TXT no importa en ARCA "Numérico Positivo" (2026-07-13, ⏳ ESPERANDO RESPUESTA DEL USUARIO)
- **Síntoma**: junio (2 quincenas) no sube a ARCA; error por cada renglón "el campo Importe del comprobante / Base de cálculo / Importe de la retención debería ser Numérico Positivo". Mayo subió OK.
- **Causa (confirmada, NO es bug de app)**: config de importación de ARCA/SICORE **9.0** trae el **Separador decimal en Punto** por defecto; el archivo usa **coma**. Mayo funcionó con Coma; al pasar a v9.0 se reseteó a Punto. Archivos mayo/junio idénticos en formato (145 chars, coma, CRLF).
- **Solución**: en ARCA cambiar Separador decimal Punto→**Coma** y re-importar. Se generaron además `exports_app/sicore_bug/GE_26_06{1,2}_..._PUNTO.TXT` (coma→punto) por si prueban con config=Punto.
- **PENDIENTE**: el usuario va a intentar subir y confirmar si funcionó (y con qué combinación). Si con Coma anda → cerrar (config de ARCA, no código). Si prefiere que la app exporte con punto → hacer el separador configurable en el generador del TXT. Detalle: KNOWLEDGE (#sicore #arca) + `memory/project_sicore_error_importacion_arca`.
