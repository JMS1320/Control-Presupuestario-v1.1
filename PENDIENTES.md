...
| A-FEAT-82 | 🔴 | Media | ⭐ **Roles creables y editables** — pedido del usuario 2026-09-05: *«Roles sí se pueden crear o editar»*. **Hoy NO se puede**: son dos, fijos en `lib/auth/roles.ts` (`type UserRole = "admin" | "contable"`), y el mapa de qué ve cada uno está en `seccionesDe()`. Para que se pueda hay que **mover roles y permisos a la base**. ⚠️ **No es sólo un CRUD, toca el modelo de seguridad** y hay que decidir 3 cosas antes de escribir código: **(1)** ¿`admin` sigue siendo intocable? Si se puede editar, alguien se quita permisos y **deja el sistema sin nadie que pueda administrarlo**, o se los agrega y escala. **(2)** ¿Dónde lee el permiso el middleware? Consultar la BD en cada request es lento; meterlo en el JWT es rápido pero **queda viejo hasta el próximo login** — un permiso revocado que sigue funcionando una hora. **(3)** ¿La grilla de permisos es por sección (12 filas) o más fina (ver / editar / borrar por sección)? Lo fino es lo que se termina queriendo, y rehacerlo después cuesta el doble. Ver `MODULO_USUARIOS.md` `@general` |
