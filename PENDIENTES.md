...
| **A-DAT-13** | 🔴 | Dato | **Cuatro vínculos entrega↔factura quedaron mal por [A-BUG-84](#a-bug-84) y [A-BUG-85](#a-bug-85)** — de las 4 entregas de maíz cargadas, **3 tienen el vínculo sin `precio_unitario`** (16/03, 06/05, 17/06) y **la del 16/03 además tiene `origen='arca'` cuando apunta a una cuota de template**. La de Longo del 24/06 quedó bien. **No se tocó nada sin preguntar.** Lo más simple es **desvincular y volver a vincular** esas tres desde el panel de Facturas, ya con el código arreglado — son tres clicks y deja el dato correcto sin un UPDATE a ciegas | → [A-DAT-13](#a-dat-13) `@productivo` |
...
| **A-DAT-14** | 🔴 | Dato | **La entrega de Longo del 24/06 quedó a $332,71/kg y el precio es $267,50** — consecuencia de [A-BUG-86](#a-bug-86). Sobre 20.100 kg son **$1.310.750** de más en el costo del maíz, que es justo el anticipo de 4,9 t que la FC 13/07 dejó pendiente de entregar. **Se corrige desvinculando y volviendo a vincular desde el panel de Facturas**, poniendo el precio **$267,50** a mano — el caso de Longo es partido y por eso va por el panel, no por la compra. Va junto con [A-DAT-13](#a-dat-13), que son los otros tres vínculos a rehacer | → [A-DAT-14](#a-dat-14) `@productivo` |
