...
| A-BUG-97 | 🔴 | Bug | **Al editar un pago de sueldo se cambia solo a «banco» aunque esté en «caja»** — *"por default. no debe cambiar nada por default. ya me ha traído problemas"*. Un default que pisa un dato ya cargado no es un default, es una pérdida silenciosa | Nota 28/08 `@sueldos` |
...
| A-BUG-99 | 🔴 | Bug | **4 débitos de sueldos AMS no concilian** — *"para el ejemplo del 30/4 yo lo tengo como pagado en sueldos. debería encontrarlo"*. Cruza con [A-BUG-33](#a-bug-33) (sueldos conciliados sin movimiento) | Nota 18/08 `@extracto @sueldos` |
| A-BUG-100 | 🔴 | Bug | **No se puede editar una fijación de arrendamiento que tiene TC puesto** y no está cerrada — *"esta venta sin querer le puse TC (…) y no puedo editar la fijación"*. Falta la edición | Nota 03/09 `@ingresos` |
| A-BUG-101 | 🔴 | **Alta** | **Crear un contrato de arrendamiento nuevo está incompleto y da error** — 3 síntomas de la misma nota: (1) **no deja elegir ni crear el proveedor**, (2) **no pide las cuotas** ni el resto de los datos *"para que quede igual que los hechos por vos a MSA"*, (3) **desde edición muestra el mismo modal incompleto** | Nota 03/09 `@ingresos` |
...
| A-TEST-89 | 🔴 | Test | **Probar el mail de Detalle de pago con echeq + transferencia** (`A-BUG-102/104/105`). Caso IGLESIAS: encolar (1) sólo la FC, (2) sólo el anticipo, (3) las dos juntas. Esperado: las tres veces la cuenta cierra en **$3.554.000** — echeq 2.454.000 + transferencia 1.042.599,60 + retención 57.400,40 — el echeq aparece **con banco, número y fecha de cobro**, el mail del anticipo solo **no promete el aviso de Galicia**, y el de los dos juntos **ya no dice $6.008.000**. **Adversario**: borrar/desvincular un medio y verificar que **se niega a encolar** diciendo cuánto falta | ← [A-BUG-104](#a-bug-104) `@cashflow` |
...
| A-TEST-97 | 🟡 | Test | ⚠️ **Al 2026-09-07 está en ROJO, 8 de 12** → `A-BUG-122`. **`npm run probar:mag`** — la ruta de precios contra el sitio real, **sólo lectura**. 12 casos: 18 categorías · **la fila «Totales» NO entra** (daría 35.210 cabezas en vez de 17.605) · `importe ÷ kilos = promedio` en todas · el `estado` · y los adversarios (fin de semana → 404 explicando, fecha mal → 400). ⚠️ Si no hay internet avisa y saltea, en vez de fallar por la red | `@ganaderia` |
...
| A-BUG-122 | 🔴 | Media | **Dos suites quedaron en ROJO y NO están investigadas** (detectado 2026-09-07 al correrlas todas; **no lo causó el trabajo de pagos** — los cambios fueron sólo `lib/pagos/`). ① **`probar:romaneo`**: lee **18 medias** donde espera 20, 1 queda sin precio y el gancho suma 3.049 en vez de 3.354 — **con el mismo PDF del 05/09, sin tocar**. Sospecha a confirmar: las expectativas se escribieron **antes** del arreglo `A-BUG-118`, que hizo que el parser **deje de adivinar**; si es eso, lo que está mal es el test, no el parser — pero **está sin verificar**. ② **`probar:mag`**: el mismo rango fijo (31/08→04/09) devuelve **17 categorías y 5.445 cabezas** donde esperaba 18 y 17.605; el parseo sigue cuadrando (`importe ÷ kilos` sin descuadres), así que **cambió el dato publicado, no la lectura** — un test que fija números vivos va a ponerse rojo solo. ⚠️ **Ninguna de las dos se miró**: son otro tema y el usuario prioriza | `@ganaderia` |
| A-FEAT-107 | 🔴 | **Alta** | **Leer la TABLA del cuerpo del mail de ARBA** (`partida · importe · link`) además del PDF. 🔑 Es un **segundo camino al mismo número**, independiente: si el importe del mail y el del PDF no coinciden, algo se leyó mal — **y se sabe sin abrir el archivo**. Es la *pieza 4* del norte administrativo, gratis. Y para el **complementario**, que no trae partida en el PDF, el cuerpo del mail es la **única** vía. Anatomía del mail → `KNOWLEDGE.md` | `@arba` |
...
| A-TEST-90 | 🔴 | Test | **Probar el ROMANEO** (`A-FEAT-94/96/97/100`). Productivo → Hacienda → 📄 Romaneo. **(1)** carga `2026-09-03 · Arre Beef SA · camión 6.500 kg`; **(2)** subir `- Romaneo.pdf`; **(3)** 5 controles en verde — `$18.750.900` · `3.354 kg` · `6.260 vivos` · `10 cab` · `53,58 %` — y **uno en rojo a propósito**: medias reses `3.049 vs 3.354` (el PDF posiciona 2 medias fuera del área legible; faltan 305 kg exactos); **(4)** desbaste: `21,0 horas`, campo `0,65 % · 0,031 %/h`, camión `3,69 % · 0,176 %/h`; **(5) 🔴 CORREGIR ANTES DE MIRAR NINGÚN RINDE** los garrones en ámbar → **509 = 234** y **512 = 373**; sin eso el apareo por peso queda corrido y **todos** los rindes son falsos. Apareo esperado: `766→373 · 562→261 · 540→257 · 522→248 · 502→234 · 478→229 · 270→146`; **(6)** rinde por grupo: `TO$5200 58,51%` · `VA$4800 52,42%` · `VA$6600 46,41%` · `VA$5800 46,20%` · `VA$5500 45,02%`; **(7)** imputación propuesta sola, `kg carne: vacío → 1.748 / 1.606`; **(8)** confirmar. **Adversarios**: romaneo de **2 hojas** (las filas no se deben mezclar entre páginas) · un PDF que **no sea** un romaneo (debe proponer vacío con avisos, **nunca romper**) · editar un valor y confirmar (se guarda el editado, no el del PDF). 📄 [Guía paso a paso](https://claude.ai/code/artifact/39b5ce68-b081-47fc-9a50-04cb0c191315) | `@ganaderia` |
...
| A-FEAT-77 | 🔴 | Feat | **Los pagos de sueldos, agrupados y colapsados POR EMPLEADO** — *"sino es difícil encontrar un dato específico. poder ver sólo del empleado que quiero"*. Pedido dos veces, el 18/08 y el 28/08 | Notas 18+28/08 `@sueldos` |
| A-FEAT-78 | 🔴 | Feat | **El detalle de sueldos pagados tiene que mostrar decimales** | Nota 28/08 `@sueldos` |
| A-FEAT-79 | 🔴 | Media | **En el pago de haberes, mostrar DE QUIÉN es cada CBU** — hoy sólo se ven los números: *"yo antes seleccionaba la palabra lucrecia (…) sino no sé cuál es cuál. lo mismo con las otras 2, una es de Santander y otra de Galicia"*. Elegir a ciegas entre CBUs es una transferencia al destinatario equivocado esperando | Nota 28/08 `@sueldos` |
...
| **A-BUG-68** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-55](#a-test-55))** · **Un input que reformatea en cada tecla es imposible de editar** — el campo de ganancia del tramo era controlado y volvía a formatear en cada `onChange`: escribías `1` y se convertía en `1,000`, el cursor saltaba y ya no podías seguir escribiendo `1,05`; borrar era peor. Lo reportó el usuario: *«no responden bien, borrar, escribir desde cero»*. Fix: **se muestra crudo mientras el campo tiene foco y se formatea al salir**. ⚠️ **Revisar si el mismo patrón está en otros campos numéricos con formato** — es el modo de falla de cualquier input controlado que formatea al vuelo | → [A-BUG-68](#a-bug-68) `@productivo` |
| **A-DAT-10** | 🟢 | Dato | **NO ERA UN ERROR — el usuario los puso a propósito (2026-08-27)** · El lote de 40 tiene **desbaste 8 % y CZ 6 %** cuando la tabla, para 371,40 kg, daría 5 % y 3 %. **Es un override deliberado** y la app se comporta bien: muestra *«tabla: 5 %»* en ámbar y ofrece aplicarla, sin pisar nada. 📌 Lo que sí quedó abierto es otra cosa: **un override legítimo deja el aviso ámbar encendido para siempre**, y un aviso que nunca se apaga termina ignorándose — habría que poder decir *«es a propósito»*. Y destapó [A-BUG-69](#a-bug-69), que era real | → [A-DAT-10](#a-dat-10) `@productivo` |
| A-TEST-55 | 🔴 | Test | **Editar la ganancia de un tramo sin pelearse** ([A-BUG-68](#a-bug-68)) — en el tramo de un lote, pararse en el campo **kg/día** y: escribir `1,05` de corrido (tiene que quedar `1,05`, no `1,000`+algo) · **borrar todo** con backspace (tiene que quedar vacío y volver al placeholder de la actividad) · escribir `0,8` empezando desde vacío · salir del campo y ver que **recién ahí** se formatea a `0,800`. Verificar que al salir se guarda y que la **curva de peso de abajo se recalcula** | → [A-TEST-55](#a-test-55) `@productivo` |
| **A-BUG-69** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-56](#a-test-56))** · **El Margen ignoraba el CZ del lote y usaba la regla general** — las ventas **proyectadas** se calculaban con `pctGastoVentaPorDefecto()` (3 % liviano / 9 % vaca-toro, los % del Excel) en vez del **`pct_cz` cargado en el lote**. Es la mitad que había quedado pendiente de [A-BUG-62](#a-bug-62), y se volvió material en cuanto el usuario ajustó un lote a **6 %**: el margen lo presentaba con **3 %** e inflaba el ingreso de Recría en ~**$2,17 M**. Fix: **el CZ del lote manda y la regla general es sólo el default**; la fila lo dice — *«6 % del lote»* vs *«3 % regla general»*. Las ventas **reales** ya usaban el CZ de `stock_ventas` | → [A-BUG-69](#a-bug-69) `@presupuesto @productivo` |
| A-TEST-56 | 🔴 | Test | **El margen usa el CZ del lote** ([A-BUG-69](#a-bug-69)) — el lote de 40 tiene **CZ 6 %** cargado a mano. En *Presupuesto → Margen → Recría*, la fila de venta proyectada tiene que descontar **6 %** y decirlo (*«del lote»*). Vaciar el `pct_cz` de un lote y verificar que vuelve a la regla general diciendo *«regla general»*. Y contrastar contra el **Presupuesto**, que ya usaba el del lote: los dos tienen que descontar lo mismo | → [A-TEST-56](#a-test-56) `@presupuesto @productivo` |
| **A-BUG-70** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-57](#a-test-57))** · **«Cargar desde una pesada» ofrecía volver a traer lo ya cargado, y podía pisarlo** — dos problemas encadenados. (1) `yaCargadas` sumaba **todos** los lotes de la categoría, incluidos los **ya vendidos**; pero el grupo son los animales **activos**, de donde los vendidos ya salieron. Con 48 machos activos y 95 cabezas en lotes (55 vendidos + 40), la resta daba 0 y la pantalla mostraba *«de 48»* como si faltara cargar todo. (2) Al aplicar, **actualizaba el lote existente** recalculando cantidad y peso desde la pesada — **pisando tramos, ganancia y desbaste ajustados a mano, sin decirlo**. Fix: `yaCargadas` cuenta sólo lotes **sin venta**, la fila muestra *«N con pesada · M ya en lotes · quedan K»*, y desde acá **sólo se crean lotes, nunca se actualiza uno**. Lo reportó el usuario: *«confunde ver las 40 cabezas, hace parecer que se va a duplicar»* | → [A-BUG-70](#a-bug-70) `@productivo` |
| A-TEST-57 | 🔴 | Test | **La pesada sólo ofrece lo que falta** ([A-BUG-70](#a-bug-70)) — en *Evolución Rodeo → Cargar stock inicial desde una pesada*, con los lotes de **55 (vendido)** y **40** cargados: el grupo de machos tiene que decir **«48 con pesada · 40 ya en lotes · quedan 8»** y ofrecer traer **8**, no 48. Un grupo con todo cargado debe mostrar **«ya está todo cargado · se edita desde el lote»** y **sin campos para traer**. Verificar que al aplicar se **crea un lote nuevo** con lo que falta y **no se toca** el lote existente — abrir el de 40 después y confirmar que conserva sus **tramos, su ganancia por tramo y su desbaste del 8 %** | → [A-TEST-57](#a-test-57) `@productivo` |
| **A-BUG-71** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-58](#a-test-58))** · **Las hembras de reposición y las de venta comparten categoría, y la app las confundía** — las dos son `Ternera Recria`; lo que las separa es el flag `es_torito` (que en las hembras marca las **retenidas**) y, del lado del lote, el **destino interno**. Sin distinguirlas pasaban dos cosas: (1) al cargar el lote de las **69 de reposición**, las **12 que sí se venden** quedaban marcadas *«ya está todo cargado»* y desaparecían de la pantalla; (2) el panel comparaba el lote de 69 contra la foto de las 12 y mostraba **«69 → 12»** y **«274,5 → 211,9»**, invitando a *corregir* un lote que estaba bien. Fix: `yaCargadas` y `desactualizado()` usan la clave `__reposicion__` cuando el lote tiene destino interno. Lo reportó el usuario | → [A-BUG-71](#a-bug-71) `@productivo` |
| **A-BUG-72** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-59](#a-test-59))** · **Un traspaso interno se valuaba con IVA en la lista de lotes** — la columna MONTO usa `valuarLoteConPrecios()`, que no sabe qué es un lote con `destino_actividad_id`: le suma **IVA 10,5 %** y le resta **CZ**. El lote de 69 de reposición vale **$109.307.902** de traspaso y la pantalla muestra **$116.959.456**. ⚠️ **En un traspaso no hay factura**: no hay IVA ni consignatario. El **Margen ya lo calcula bien** (`TransferenciaInterna` no aplica ninguno de los dos) y el **Cash Flow ya lo excluye** — el que quedó afuera del arreglo es el listado. Además el lote tiene **CZ 4 % cargado**, que para un traspaso es dato muerto y conviene que la pantalla lo diga | → [A-BUG-72](#a-bug-72) `@productivo` |
| A-TEST-58 | 🔴 | Test | **Reposición y venta no se pisan** ([A-BUG-71](#a-bug-71)) — con el lote de **69 de reposición** cargado (destino `Cria`), abrir *Cargar desde una pesada*: el grupo de **hembras marcadas** tiene que decir *«69 con pesada · 69 ya en lotes · ya está todo cargado»*, y el de **hembras sin marcar** *«12 con pesada · quedan 12»* — **no** *«ya está todo cargado»*. Y en la lista de lotes, el de 69 **no debe mostrar el aviso de desactualizado** (`69 → 12`). Probar el caso inverso: cargar las 12 de venta y verificar que el grupo de reposición **no cambia** | → [A-TEST-58](#a-test-58) `@productivo` |
| **A-DEC-09** | 🟢 | Decisión | **RESUELTA 2026-08-27** · **Un traspaso interno lleva la comercialización descontada, pero NO el IVA** — regla del usuario: *«sí se debe descontar la comercialización, ya que si recría vende ésta se resta — entonces cría no debe pagar por ello. Ídem el destete que sale de cría a recría»*. El precio del traspaso es **de mercado**, así que para que la operación sea neutra para el que entrega hay que netearlo de lo que el mercado se habría llevado. **El IVA no va porque no hay factura**: pasárselo al que recibe sería cargarle un impuesto que nadie pagó. Aplica a las dos puntas — destete y reposición | → [A-DEC-09](#a-dec-09) `@productivo @presupuesto` |
| **A-FEAT-57** | 🟡 | Feat | **HECHO 2026-08-27 — falta testear ([A-TEST-59](#a-test-59))** · ⚠️ **Y hoy NO ofrece nada, que es lo correcto**: los 16 que parecían faltar resultaron ser toros y vacas CUT ([A-BUG-73](#a-bug-73)). · **Los animales que NUNCA se pesaron se pueden cargar** — quedaban afuera del stock y nadie los reclamaba. ⚠️ **No son «los que faltaron en la última»** — el modo *última pesada de cada animal* ya rescata a los que se pesaron alguna vez; éstos no tienen ninguna. Se ofrecen en **una fila propia por grupo**, marcada *«nunca pesados — van al promedio del grupo»*, y **aparte** de los pesados porque sin pesos individuales el selector de *los más pesados* no significa nada | → [A-FEAT-57](#a-feat-57) `@productivo` |
| A-TEST-59 | 🔴 | Test | **Los nunca pesados y el valor del traspaso** ([A-FEAT-57](#a-feat-57) + [A-BUG-72](#a-bug-72) + [A-DEC-09](#a-dec-09)) — en *Cargar desde una pesada* tienen que aparecer **dos filas nuevas**: *«8 sin pesada»* de machos y *«8 sin pesada»* de hembras, con la chapa ámbar y el peso promedio de su grupo. Cargarlas y verificar que el *«Resto sin lote»* del reparto **baja**. Y en la lista de lotes, el de **69 de reposición** tiene que mostrar **$104,9 M** —el neto del traspaso, sin IVA y con la comercialización descontada— y no los **$117,0 M** de antes, con la chapa *traspaso*. Contrastar contra el **Margen**: los dos tienen que decir lo mismo | → [A-TEST-59](#a-test-59) `@productivo @presupuesto` |
| **A-BUG-73** | 🔴 | Bug | **`productivo.terneros` no son sólo terneros, y la pantalla de pesadas los mezclaba** — ahí viven también **8 toros nacidos en 2024** y **8 vacas CUT del tacto**. La pantalla agrupa por `sexo` + `es_torito` e **ignora `categoria_id`**, que sí los distingue bien: los toros se disfrazaban de *Ternero Recria* y las vacas CUT de *Ternera Recria*. ⚠️ **Se descubrió porque el usuario preguntó de dónde salían esos «16 nunca pesados»** — la pregunta era la correcta. Parche puesto (2026-08-27): se filtra por categoría y los ajenos **se muestran contados aparte** en vez de esconderse. **Falta lo de fondo**: agrupar por `categoria_id` y no por sexo. 📌 Y hay **2 animales donde `es_torito` y `categoria_id` se contradicen** (`Torito` da 7 por categoría y 9 por el flag) | → [A-BUG-73](#a-bug-73) `@productivo` |
| A-TEST-60 | 🔴 | Test | **Los ajenos no se ofrecen como recría** ([A-BUG-73](#a-bug-73)) — en *Cargar desde una pesada* **no** debe aparecer ninguna fila *«8 sin pesada»* de machos ni de hembras. En su lugar, un aviso celeste: *«No se ofrecen acá, porque no son de recría: 8 Toro · 8 Vaca CUT/Descarte»*. Verificar que las filas de **con pesada** no cambiaron: *Ternera Recria 12* y *69 marcadas*, *Ternero Recria 40*, *Torito 9* | → [A-TEST-60](#a-test-60) `@productivo` |
| **A-BUG-74** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-61](#a-test-61))** · **Un grupo ya completo se podía tildar igual, y aceptar habría creado un lote duplicado** — en *Cargar desde una pesada* la fila decía *«ya está todo cargado»* pero **el checkbox seguía habilitado**. Y como desde [A-BUG-70](#a-bug-70) esa pantalla ya **no actualiza, sólo inserta**, aceptar habría creado un segundo lote de las mismas cabezas. **El usuario lo vio venir y no apretó**: *«no le di aceptar para no hacer lío, pero no sé qué pasaría si los genero de nuevo»*. Fix: checkbox **deshabilitado** sin cabezas por traer, más dos cinturones — `aplicar()` saltea los grupos completos y la cantidad se **clampea** a lo que falta | → [A-BUG-74](#a-bug-74) `@productivo` |
...
| **A-BUG-77** | 🟡 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-63](#a-test-63))** · **La compra de insumos usaba `type="number"` para cantidad y costo, contra la convención del proyecto** — `CLAUDE.md` § *Convención Inputs Monetarios (es-AR)* dice **`type="text"` siempre, nunca `type="number"` para montos**, y el modal *Nueva Compra de Insumos* (`vista-sector-productivo.tsx`) los tiene los dos como `number`. Consecuencia práctica: hay que escribir **`238.35` con punto**, no `238,35` — al revés de todo el resto de la app. Con precios de maíz de 4 y 5 cifras y decimales es fácil equivocarse y que quede **un precio mil veces mayor** sin que nada avise | → [A-BUG-77](#a-bug-77) `@productivo` |
| A-TEST-63 | 🔴 | Test | **La compra de insumos acepta coma decimal** ([A-BUG-77](#a-bug-77)) — en *Insumos → Stock → + Compra*, cargar una línea con **`5.960`** de cantidad y **`238,35`** de costo: tiene que guardar **5960** y **238,35**, no 5 y 238. Verificar que el **stock del producto sube 5.960**, no 5. Probar también `1.740` y `267,5`. ⚠️ Y que **no se rompió lo que ya andaba**: cargar `100` y `50` a secas tiene que seguir funcionando | → [A-TEST-63](#a-test-63) `@productivo` |
| **A-BUG-78** | 🟡 | Bug | **Quedan otros `type="number"` con decimales en el sector productivo** — en `vista-sector-productivo.tsx`, las líneas de las **órdenes de aplicación** (dosis, cantidades) siguen con `type="number" step="0.01"`. Los que son **enteros** (años, cabezas, terneros nacidos) están bien así — la convención de `CLAUDE.md` es sobre **montos y decimales**. Se separó de [A-BUG-77](#a-bug-77) para no tocar pantallas que no se están probando: **cambiar un parser a ciegas es cómo se rompen las que ya andaban** | → [A-BUG-78](#a-bug-78) `@productivo` |
| **A-BUG-79** | 🟡 | Bug | **La vista de EGRESOS SIN FACTURA también tiene el problema de la coma** — reportado por el usuario el 2026-08-28 mientras se arreglaba el mismo caso en insumos ([A-BUG-77](#a-bug-77)). ⚠️ **NO se toca desde acá**: esas pantallas las está trabajando **otra terminal**, y cambiar un parser en un archivo ajeno es exactamente lo que la § *Trabajo en paralelo* de `CLAUDE.md` prohíbe. Queda anotado para quien las tenga tomadas. 📌 **El arreglo ya está hecho dos veces y es siempre el mismo**: `type="text"` + `inputMode="decimal"` y **`parseNumeroAR()`** de `lib/format/numero` — el parser único, que entiende `5.960` y `238,35`. No hace falta escribir nada nuevo, sólo usarlo | → [A-BUG-79](#a-bug-79) `@egresos` |
...
| A-TEST-66 | 🔴 | Test | **Vincular una entrega con un template** ([A-FEAT-61](#a-feat-61)) — en *Insumos → Facturas* del **Maíz Granel**, en la entrega del **16/03 · 1.740 kg**, buscar `maiz`: tiene que aparecer la cuota marcada **`TEMPLATE`** en celeste, con la descripción *«1.740 Kg Maiz Castillo a 193.000 la ton»* y **$335.800**. Vincularla por **1.740 kg** y verificar que el precio de la entrega pasa a **«de las facturas»** y que el control **«Lo entregado tiene respaldo»** deja de marcar esos 1.740 kg. Probar también desde *+ Compra*: el buscador de la línea tiene que ofrecer **los dos orígenes**, con el template destacado | → [A-TEST-66](#a-test-66) `@productivo @egresos` |
| **A-BUG-81** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-67](#a-test-67))** · **El buscador de respaldos no encontraba nada viejo: un TOPE SILENCIOSO** — precargaba *las últimas 400* de cada origen y filtraba en el navegador. Hay **1.019 cuotas de template**, y la del maíz del 16/03 tiene **591 más nuevas encima**: nunca se cargaba. El usuario la buscó por `maiz`, por `otros gastos` y por proveedor, y no aparecía **porque la fila no estaba**. Fix: `buscarRespaldos()` **busca en el servidor** (`ilike` sobre proveedor, número y descripción), con 250 ms de respiro. 🔑 **La lección**: un tope silencioso no da error, **da menos resultados** — y eso no se distingue de *«no existe»* | → [A-BUG-81](#a-bug-81) `@productivo` |
| **A-BUG-82** | 🟡 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-67](#a-test-67))** · **Una factura ya aplicada entera se seguía ofreciendo** — el usuario cargó la primera de Arroyo Tala y al cargar la segunda entrega el buscador **le volvía a ofrecer las dos**. Vincularla otra vez la contaría dos veces. Fix: `estaAplicadaEntera()` la saca de la lista, y si todas las coincidencias están aplicadas **lo dice** en vez de mostrar vacío. Se compara **por monto**, no por cantidad: la cantidad del comprobante no está en ningún lado, lo único que se sabe es su neto. Tolerancia del 0,5 %, porque los precios se cargan redondeados | → [A-BUG-82](#a-bug-82) `@productivo` |
| **A-BUG-83** | 🟡 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-67](#a-test-67))** · **El selector de insumo no dejaba escribir** — era un `Select` cerrado: click y a recorrer 30 productos con el mouse. Lo marcó el usuario: *«cuando hago click me abre selector y no me deja escribir; para escribir debo ir contab»*. Fix: buscador con texto, como el de proveedores, con `normalizarBusqueda()` — escribir `maiz` encuentra `Maíz Granel`. Si no hay coincidencias sugiere revisar el **filtro Ganadero/Agrícola**, que es la causa habitual | → [A-BUG-83](#a-bug-83) `@productivo` |
| A-TEST-67 | 🔴 | Test | **El buscador de insumos y de respaldos** ([A-BUG-81](#a-bug-81) + [A-BUG-82](#a-bug-82) + [A-BUG-83](#a-bug-83)) — en *Insumos → + Compra*: el campo **Insumo** tiene que dejar **escribir** (`maiz` → *Maíz Granel*). En **Factura**, buscar `maiz` tiene que traer la cuota **TEMPLATE** del 16/03 aunque tenga 591 cuotas más nuevas encima. Cargar la entrega del **06/05** vinculando la **FC 0001-00000024**; después, al cargar la del **02/06**, esa factura **ya no debe aparecer** — y si se busca `arroyo` tiene que salir sólo la **0001-00000025** | → [A-TEST-67](#a-test-67) `@productivo` |
| **A-BUG-96** | 🟡 | Bug | **HECHO 2026-08-31 — falta testear** · **El error de "Agrupar" acusaba a la causa equivocada.** Al intentar agrupar el pago de haberes de **Sigot + Wilson** (el banco los debitó juntos), la app decía *"Seleccioná al menos 2 filas individuales **del mismo proveedor**"* — y el proveedor no tenía nada que ver: la fila de Sigot **ya era un grupo**, el filtro `facturas_agrupadas <= 1` la descartaba y quedaba **1 sola fila**. El usuario se puso a mirar los CUITs, que era el camino equivocado. Fix: el mensaje distingue los dos casos y, cuando hay filas ya agrupadas, **dice que hay que desagrupar primero**. 📌 Nota: agrupar **CUITs distintos SÍ se puede** — sólo pide confirmación (`⚠️ tienen CUITs diferentes, ¿agrupar igual?`) y lo deja anotado en `observaciones`; lo que está bloqueado es mezclar **empresas** en facturas ARCA (hay FK) | → [A-BUG-96](#a-bug-96) `@cashflow` |
| **A-BUG-95** | ✅ | **Bug** | **HECHO Y TESTEADO OK 2026-08-31** ([A-TEST-80](#a-test-80)) · **El mail de Detalle de pago de un ANTICIPO con retención no la mencionaba ni adjuntaba el certificado.** Caso: Genoil Co SA, $11.100.000 con **$154.752,79** de Ganancias — el mail decía sólo *"Total transferido: $10.945.247,21"*, sin el renglón de la retención. **Tres causas encadenadas**: (1) la fila ANTICIPO del Cash Flow **no llevaba `monto_sicore` ni `imp_total`** (sólo `debitos`, que ya venía neto) → el cuerpo calculaba `totalRet = 0` y omitía el desglose; (2) todo el bloque del certificado estaba detrás de un **`if (tipo === 'arca')`**, y un anticipo solo cae en `'template'`; (3) aun entrando, `sicore_retenciones` se consultaba **sólo por `factura_id`**, que en una retención de anticipo es `NULL` — se vincula por **`anticipo_id`**. El registro existía completo (certificado **00002026000056**): nadie lo buscaba. 🔍 **Y de paso apareció un cuarto**, latente: `schemaDeFila()` devolvía **`'anticipos_proveedores'` como si fuera un schema** (partía `origen_tabla` por `.` sin validar), así que la consulta habría ido a un schema inexistente **sin error** — mismo modo de falla que A-FEAT-13. Ahora valida contra los schemas reales. ⚠️ Verificado que **todas** las llamadas a `esFilaMsa()` están guardadas por `origen === 'ARCA'`, así que el cambio no altera ninguna ruta que funcionara | → [A-BUG-95](#a-bug-95) `@cashflow` |
| A-TEST-80 | 🟢 | Test | **TESTEADO OK 2026-08-31** — el mail de **Genoil Co SA** salió con el desglose completo (`Importe $11.100.000 · Retención Ganancias −$154.752,79 · Total transferido $10.945.247,21`), la línea *"Se practicó retención…"* y el **certificado adjunto** (12.676 bytes, `tiene_sicore` y `adjuntar_retencion` en true). Verificado en `mails_pago`. 📌 Al mirar el mail apareció un detalle de redacción ya corregido: decía **"Importe facturas"** en un anticipo, que no tiene facturas. ⏳ **Falta un caso**: pago **mixto** (facturas + anticipo del mismo proveedor) — los certificados de **ambos** orígenes tienen que salir en el mismo PDF | → [A-BUG-95](#a-bug-95) `@cashflow` |
| **A-BUG-94** | ✅ | **Bug** | **HECHO Y TESTEADO OK 2026-08-31** ([A-TEST-79](#a-test-79)) · **No se podía pasar a pagado un GRUPO de pagos de sueldo.** Lo encontró el usuario con el grupo de **Ruben Sigot, Anticipo Ago 2026** (2 pagos, $1.162.760). Dos fallas encadenadas: **(1)** el guard de `actualizarRegistro` decía `origen_tabla !== 'sueldos.pagos'` → **rechazaba el grupo entero, en cualquier campo**, confundiendo un **período** (que sí es de sólo lectura) con un **grupo** (que es N filas de `sueldos.pagos`, editable como cualquier pago); **(2)** aun pasando el guard, el UPDATE iba por `.eq('id', id)` y en una fila-grupo el `id` es el **`grupo_pago_id`**, que no existe en `sueldos_pagos` → **0 filas**. Fix: el guard acepta `msa.grupos_pago` y se escribe sobre `ids_grupo`, el mismo patrón que ARCA y TEMPLATE ya usaban. Corregido además el aviso, que decía *"el sueldo del mes se gestiona desde Sueldos"* — falso para un grupo, y mandaba al usuario al lugar equivocado. ⚠️ **Es el hueco que dejó [A-BUG-52](#a-bug-52)**: aquel fix arregló la fila individual y **no miró la agrupada** — los grupos viejos figuran `conciliado` porque llegaron por el motor, no por este botón, así que nadie lo había notado | → [A-BUG-94](#a-bug-94) `@cashflow` |
| A-TEST-79 | 🟢 | Test | **TESTEADO OK 2026-08-31** — el grupo de **Ruben Sigot Anticipo Ago 2026** pasó a pagado desde el Cash Flow. Verificado en BD: **los 2 miembros** en `pagado`, $1.162.760, ninguno quedó atrás. ⏳ **Falta un caso**: que un **período** de sueldo **siga rechazándose** con el aviso de que se gestiona desde Sueldos — es la mitad que protege el guard, y no se probó | → [A-BUG-94](#a-bug-94) `@cashflow` |
| A-TEST-78 | 🔴 | Test | **Dar de alta un proveedor, por las dos vías** ([A-BUG-93](#a-bug-93)) — **(a)** *Principal → Proveedores*: buscar algo que no exista → tiene que aparecer **«+ Dar de alta …»**; crearlo y que **se abra su ficha** para cargarle CBU y mail. Probar también el botón **+ Nuevo** de arriba. **(b)** Cargar un **anticipo** con un CUIT nuevo → avisa *"Se dio de alta …"* y el proveedor aparece en la lista. **(c)** Repetir con un CUIT **que ya existe**: NO debe pisar la razón social, sólo decir que ya estaba (y marcar el flag si faltaba). **(d)** Anticipo **sin CUIT**: se guarda igual y avisa que la contraparte no se dio de alta | → [A-BUG-93](#a-bug-93) `@egresos` |
| **A-BUG-93** | 🟡 | **Bug** | **HECHO 2026-08-31 — falta testear ([A-TEST-78](#a-test-78))** · 🔴 **«Cargar nuevo proveedor» NO creaba el proveedor — y NO había ninguna otra forma de crearlo a mano.** Lo encontró el usuario 2026-08-31 cargando un anticipo a **Genoil Co SA** ($11,1 M): el anticipo se guardó, el proveedor no. El modo *nuevo* de `ProveedorCombobox` (`:133-157`) es **sólo UI**: escribe CUIT + razón social en el formulario vía `onChange` y **nunca toca `public.proveedores`**. Y el **único** `INSERT` a esa tabla en todo el repo es el importador de ARCA (`import-facturas-arca/route.ts:638`) — la pantalla de config escribe por `PATCH`, que sólo actualiza. **Consecuencia**: sin la fila en el maestro no hay dónde cargar **mail ni CBU**, así que el proveedor no entra al export de lotes de pago ni al mail de detalle. Viola `CLAUDE.md` § Contrapartes, que dice explícitamente *"y para todas las vías: importadores masivos **y altas manuales**"*. **No es de hoy: 4 anticipos** tienen CUIT fuera del maestro desde feb-2026 (2 con placeholder `11111111111`). Hermano de [A-FEAT-41](#a-feat-41) y [B-BUG-CLIENTE-NO-SE-CREA](#b-bug-cliente-no-se-crea) — los tres son **la misma regla rota en tres vías distintas**. **✅ Fix (2026-08-31)**: helper único **`lib/proveedores/alta.ts`** (find-or-create, nunca pisa) + **`POST /api/gas/config-proveedor`**, que ya era la única vía de escritura del maestro. Se conectaron **las 2 vías que pidió el usuario**: el **alta de anticipo** (sólo si hay CUIT — decisión suya) y **Principal → Proveedores** (botón *+ Nuevo* y *«+ Dar de alta …»* cuando el buscador no encuentra). ⏳ **A-FEAT-41 y B-BUG-CLIENTE-NO-SE-CREA quedan abiertos**, pero ahora es enchufarles el mismo helper. **Alta manual con OK del usuario**: Genoil Co SA (33708128789) | → [A-BUG-93](#a-bug-93) `@egresos` |
| **A-FEAT-63** | 🟡 | Feat | **Normalizar los buscadores: uno solo, y siempre igual** — pedido del usuario 2026-08-28: *«estas cosas ya deben irse quedando siempre iguales»*. Hoy conviven **tres patrones** para elegir algo de una lista: el `Select` cerrado (sin escribir), el `ProveedorCombobox` (bien) y buscadores escritos a mano en cada pantalla. Hay que **elegir uno y pasarlos todos**. Va junto con [A-FEAT-62](#a-feat-62) (los campos de proveedor). ⚠️ **No es cosmético**: un `Select` de 30 ítems obliga a recorrerlos con el mouse, y con 100 se vuelve inusable — es lo que acaba de pasar con los insumos | → [A-FEAT-63](#a-feat-63) `@general` |
| **A-FEAT-64** | 🟡 | Feat | **Auditar los campos donde no se llega a ver lo que se escribe** — pedido del usuario 2026-08-28: *«suele haber temas de espacios reducidos donde no se llega a ver bien lo que se escribe o selecciona»*. Ya aparecieron dos casos concretos y se arreglaron sueltos: la columna **«Ha»** del tramo (`w-16`, ilegible, se terminó sacando) y el **selector de insumo** en la compra. 📌 **Lo que se puede auditar solo**: buscar anchos fijos chicos (`w-14`, `w-16`, `w-20`) en inputs que reciben texto o números largos. **Lo que no**: si en la pantalla real se ve bien — eso necesita el ojo del usuario. Conviene hacer la lista automática primero y revisarla juntos | → [A-FEAT-64](#a-feat-64) `@general` |
| **A-BUG-84** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-68](#a-test-68))** · **El vínculo con la factura se guardaba SIN PRECIO, y de ahí salían otros dos síntomas** — el precio se derivaba del respaldo **sólo al hacer click en él**, así que elegir la factura **antes** de escribir la cantidad lo dejaba en `null`. Sin precio, `estaAplicadaEntera()` calculaba **cero aplicado** y la factura se seguía ofreciendo como si estuviera libre — que es lo que el usuario reportó: *«me siguen apareciendo las 2 facturas de arroyo tala… pero una sola de longo»*. La de Longo sí desaparecía porque ésa **sí** tenía precio. Fix: el precio se deriva **al guardar**, y `usoDeRespaldo()` distingue `libre` / `parcial` / `entera` / **`sin precio`**, que ahora se dice en pantalla en vez de tratarse como libre | → [A-BUG-84](#a-bug-84) `@productivo` |
| **A-BUG-85** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-68](#a-test-68))** · **Un template elegido por búsqueda se guardaba con `origen = 'arca'`** — el origen se buscaba **sólo en la lista precargada**, y los resultados de la búsqueda en el servidor viven en otro estado. Consecuencia: el vínculo apuntaba a una cuota pero decía ser un comprobante, así que **no resolvía contra ninguna tabla** y quedaba huérfano. Ya pasó con la entrega del **16/03**: tiene vínculo, `origen='arca'` y ningún comprobante detrás. ⚠️ **Hay que reparar esa fila** — ver [A-DAT-13](#a-dat-13) | → [A-BUG-85](#a-bug-85) `@productivo` |
| **A-DAT-13** | 🟢 | Dato | **REPARADOS 2026-08-28, con OK del usuario (3 de 4 — Pereyra queda como está por pedido suyo)** · **Cuatro vínculos entrega↔factura quedaron mal por [A-BUG-84](#a-bug-84) y [A-BUG-85](#a-bug-85)** — de las 4 entregas de maíz cargadas, **3 tienen el vínculo sin `precio_unitario`** (16/03, 06/05, 17/06) y **la del 16/03 además tiene `origen='arca'` cuando apunta a una cuota de template**. La de Longo del 24/06 quedó bien. **No se tocó nada sin preguntar.** Lo más simple es **desvincular y volver a vincular** esas tres desde el panel de Facturas, ya con el código arreglado — son tres clicks y deja el dato correcto sin un UPDATE a ciegas | → [A-DAT-13](#a-dat-13) `@productivo` |
| A-TEST-68 | 🔴 | Test | **El vínculo guarda el precio y el origen** ([A-BUG-84](#a-bug-84) + [A-BUG-85](#a-bug-85)) — en *+ Compra*, elegir **primero la factura y después escribir la cantidad**: al guardar, el vínculo tiene que quedar **con precio** (verificar en el panel de *Facturas*: el precio de la entrega debe decir **«de las facturas»**). Cargar la entrega del **02/06** con la **FC 0001-00000025** y confirmar que después **esa factura ya no se ofrece**. Elegir un **TEMPLATE** por búsqueda y verificar que el vínculo queda marcado *Template*, no *FC*. Y que una factura **parcialmente usada** aparece con *«ya usada · queda $X»* en vez de desaparecer | → [A-TEST-68](#a-test-68) `@productivo` |
| **A-BUG-86** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-69](#a-test-69))** · **Derivar el precio como `neto ÷ cantidad entregada` da MAL cuando la factura cubre más de lo entregado** — y no avisaba. Caso real: la FC de Longo del 13/07 facturó **25 t** y esa entrega fue de **20,1 t**; la división dio **$332,71/kg** cuando el precio es **$267,50**. La diferencia, **$1.310.750**, es exactamente **el anticipo de 4,9 t** — o sea que el error se comió el anticipo metiéndolo en el precio. Y el control *«lo facturado está aplicado»* **quedaba conforme**, porque 20.100 × 332,71 da justo el neto. Fix: la **división se muestra** (`$6.687.500 ÷ 20.100 = 332,71`) con la pregunta *«¿la factura cubre sólo esta entrega?»*, y el panel avisa cuando **una entrega quedó a un precio muy distinto** del resto del mismo insumo. 🔑 **El dato que falta es la cantidad que declara la factura**, y no está en ningún lado | → [A-BUG-86](#a-bug-86) `@productivo` |
| **A-DAT-14** | 🟢 | Dato | **REPARADO 2026-08-28, con OK del usuario** · **La entrega de Longo del 24/06 quedaba a $332,71/kg y el precio es $267,50** — consecuencia de [A-BUG-86](#a-bug-86). Sobre 20.100 kg son **$1.310.750** de más en el costo del maíz, que es justo el anticipo de 4,9 t que la FC 13/07 dejó pendiente de entregar. **Se corrige desvinculando y volviendo a vincular desde el panel de Facturas**, poniendo el precio **$267,50** a mano — el caso de Longo es partido y por eso va por el panel, no por la compra. Va junto con [A-DAT-13](#a-dat-13), que son los otros tres vínculos a rehacer | → [A-DAT-14](#a-dat-14) `@productivo` |
| A-TEST-69 | 🔴 | Test | **La división del precio se ve y se puede cuestionar** ([A-BUG-86](#a-bug-86)) — en *+ Compra*, al elegir una factura con la cantidad cargada tiene que aparecer debajo del costo la cuenta: **`$1.912.600 ÷ 7.300 = 262`** y la pregunta *«¿la factura cubre sólo esta entrega?»*. Y en el panel de *Facturas* del Maíz, con las 6 entregas cargadas, si una queda a un precio **más de 20 % distinto de la mediana** tiene que salir el aviso ámbar nombrándola. Verificar que **con los precios correctos el aviso NO aparece** — un control que salta siempre no sirve | → [A-TEST-69](#a-test-69) `@productivo` |
| **A-FEAT-65** | 🟡 | Feat | **HECHO 2026-08-28 — falta testear ([A-TEST-70](#a-test-70))** · **Los vínculos entrega↔factura se editan en el lugar** — antes sólo se podían **borrar**, así que corregir un precio obligaba a desvincular y volver a buscar la factura. Lo preguntó el usuario: *«¿los puedo sólo editar? No hace falta borrar y rehacer»*. Ahora **cantidad y precio son editables inline**, con el borrador por campo para que no reformatee en cada tecla —el problema de [A-BUG-68](#a-bug-68)— y con el **subtotal a la derecha**, que es lo que hace visible si la suma de los pedazos da el neto de la factura | → [A-FEAT-65](#a-feat-65) `@productivo` |
| A-TEST-70 | 🔴 | Test | **Editar un vínculo sin rehacerlo** ([A-FEAT-65](#a-feat-65)) — en el panel de *Facturas* del Maíz, en la entrega del **24/06**, cambiar el precio del vínculo de **332,71** a **267,50**: al salir del campo tiene que guardarse y el **subtotal** pasar de $6.687.500 a **$5.376.750**. Verificar que el control *«Lo facturado está aplicado»* **pasa a marcar la diferencia** de $1.310.750 —el anticipo—, que es lo correcto hasta que se vincule el pedazo de la entrega del 24/07. Probar también editar la **cantidad**, y que escribir `267,5` de corrido **no se reformatee a media palabra** | → [A-TEST-70](#a-test-70) `@productivo` |
| **A-BUG-87** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-71](#a-test-71))** · **El control «Lo facturado está aplicado» sumaba TODAS las facturas del sistema** — comparaba el neto de los **300 comprobantes precargados** contra lo imputado a un solo insumo. Daba una diferencia enorme y constante, o sea que **no decía nada**: por eso el usuario no encontraba el aviso del anticipo de Longo (*«no sé dónde se ve eso»*). Tenía razón — **no se veía porque no significaba nada**. Fix: el control mira **sólo los respaldos vinculados**, y se agregó el **detalle por respaldo** (*«FC 0002-00000516 · 20.100 kg imputados · $5.376.750 de $6.687.500 · quedan $1.310.750»*). 🔑 El total avisa; **la lista es la que deja arreglarlo** | → [A-BUG-87](#a-bug-87) `@productivo` |
| A-TEST-71 | 🔴 | Test | **El control de facturas señala la que falta** ([A-BUG-87](#a-bug-87)) — en el panel de *Facturas* del Maíz, con los 4 vínculos cargados, el bloque **«Respaldos vinculados»** tiene que listar **cuatro** y marcar en ámbar **sólo la FC 0002-00000516** con *«quedan $1.310.750»*. Los otros tres deben decir el monto sin aviso. Verificar que el control de arriba dice **la misma cifra** y no una diferencia de cientos de millones. Después de vincular los 4.900 kg a la entrega del 24/07, el aviso **tiene que apagarse** | → [A-TEST-71](#a-test-71) `@productivo` |
| **A-BUG-88** | 🔴 | Bug | **HECHO 2026-08-28 — falta testear ([A-TEST-72](#a-test-72))** · **El control global cerraba con DOS ERRORES QUE SE COMPENSABAN** — caso real, aparecido al terminar de cargar el maíz: la FC de Longo del 13/07 quedó imputada **de menos por $1.310.750** y la del 14/08 **de más por $1.310.795**. La diferencia global daba **$45** y el control decía que todo cerraba. 🔑 **Un total que cierra no prueba que las partes estén bien** — es el mismo modo de falla que ya nos había mordido en el reparto de la maqueta. Fix: el control exige que **cada respaldo cierre uno por uno**, no sólo la suma; y el detalle distingue *«quedan $X»* de **«imputado de MÁS por $X»**, que son problemas opuestos | → [A-BUG-88](#a-bug-88) `@productivo` |
| **A-DAT-15** | 🟢 | Dato | **REPARADO 2026-08-28, con OK del usuario · LOS 7 RESPALDOS CIERRAN** · **La entrega de Longo del 24/07 estaba vinculada a UNA sola factura y son dos** — el usuario la cargó completa (25.000 kg) contra la **FC 14/08** al precio ponderado **$267,14**, que da **$6.678.500** contra un neto de **$5.367.705**. El ponderado era el **resultado** del reparto, no un precio de factura. Lo correcto son **dos vínculos**: **4.900 kg × $267,50** de la FC 13/07 (el anticipo) y **20.100 kg × $267,05** de la FC 14/08. Con eso las dos facturas cierran exactas y la entrega queda igual a $267,14 — pero **calculado**, no supuesto. **No se tocó**: hay que hacerlo o autorizarlo | → [A-DAT-15](#a-dat-15) `@productivo` |
| **A-DOC-11** | 🟢 | Doc | **La confusión que vale la pena tener escrita: un respaldo parcial NO es una compra nueva** — al explicarle el arreglo de Longo, el usuario preguntó *«¿o sea que es hacer una compra nueva de 4900 para la factura de 25T?»*. **No**: crear una compra de 4.900 kg habría subido el stock de **66.660 a 71.560** con mercadería que nunca llegó. Las 4,9 t **ya están dentro** de la entrega de 25.000 del 24/07; lo que faltaba era un **segundo respaldo para esa misma entrega**. La analogía que lo cerró: *como pagar una compra con dos cheques — son dos comprobantes, una sola compra*. 📌 Vale registrarla porque **es la pregunta natural**: el modelo de muchos-a-muchos no es obvio, y equivocarse acá inventa stock sin que nada avise | → [A-DOC-11](#a-doc-11) `@productivo` |
| A-TEST-72 | 🔴 | Test | **El control no se deja engañar por errores que se compensan** ([A-BUG-88](#a-bug-88)) — con la entrega del 24/07 mal vinculada (25.000 kg a la FC 14/08), el control de arriba tiene que decir **«2 de 6 respaldos no cierran»** y **no** «todo cierra», aunque la diferencia global sea de $45. En el detalle, la FC 13/07 debe decir *«quedan $1.310.750»* en ámbar y la 14/08 *«imputado de MÁS por $1.310.795»* en **rojo**. Al partir el vínculo en dos, **los seis tienen que quedar sin aviso** | → [A-TEST-72](#a-test-72) `@productivo` |
| **A-BUG-89** | 🔴 | Bug | **HECHO 2026-08-29 — falta testear ([A-TEST-73](#a-test-73))** · **Un placeholder `0` idéntico al valor más común, y un botón que no hacía nada** — en *Mediciones* el campo *«Había (kg)»* tenía `placeholder="0"`. Pero **en una medición cero ES el valor más común** —el silo vacío al recibir un camión—, así que el usuario vio el `0`, creyó que estaba cargado, apretó `+` y **no pasó nada**: el campo estaba en blanco, el botón deshabilitado y **nada lo decía**. Fix: el placeholder pasa a `«kg de …»`, que no se puede confundir con un dato; el botón **ya no se deshabilita en silencio** sino que dice qué falta (*«Si el silo estaba vacío, escribí 0 — el campo está en blanco»*). Mismo cambio en el campo de cantidad de la compra | → [A-BUG-89](#a-bug-89) `@productivo` |
| A-TEST-73 | 🔴 | Test | **El placeholder no se confunde con el dato** ([A-BUG-89](#a-bug-89)) — en *Mediciones* del Maíz, el campo de cantidad tiene que decir **«kg de kg»** en gris, no `0`. Apretar `+` con el campo vacío tiene que **avisar qué falta**, no quedarse mudo. Cargar `16/03/2026` con **`0`** escrito a mano y verificar que **se guarda y aparece en la lista**. Repetir con las otras tres del maíz | → [A-TEST-73](#a-test-73) `@productivo` |
| **A-BUG-90** | 🔴 | Bug | **HECHO 2026-08-29 — falta testear ([A-TEST-75](#a-test-75))** · **El mismo tope silencioso de [A-BUG-81](#a-bug-81), pero del lado de la LECTURA — y se comía un tramo entero: 21.560 kg, el **35 % de los kilos** y **$5,35 M** · Con las 6 mediciones ya cargadas, el Margen seguía sin mostrar el maíz del primer tramo. La cadena estaba perfecta: el problema era que **tres pantallas leían un vínculo ya hecho con la lista de «los últimos N»**. El respaldo del maíz del 16/03 es una **cuota de template con 591 más nuevas encima** — nunca entraba en la lista, el vínculo quedaba **sin precio**, y el tramo entero (**21.560 kg**) salía del costo **sin decir nada**. Peor: `panel-margen.tsx` tenía **su propia query** a `comprobantes_arca` con `limit(400)` — así que **ni siquiera miraba los templates**. Fix: `respaldosPorId()` nuevo en el lib, que trae **exactamente los vinculados**, de los dos orígenes y sin tope posible; lo usan el margen, el panel de entregas y el modal de compra. 🔑 **La distinción que hay que tener escrita**: `traerRespaldos()` es para **ofrecerle una lista al usuario** y por eso tiene tope; `respaldosPorId()` es para **leer un vínculo que ya existe** y por eso no puede tenerlo. Confundirlas no da error — da un costo de menos | → [A-BUG-90](#a-bug-90) `@productivo` |
| A-TEST-75 | 🔴 | Test | **El Margen muestra el costo de alimentación medido** ([A-BUG-90](#a-bug-90)) — en *Presupuesto → Margen*, campaña **25/26**: la fila de Recría tiene que decir **Maíz Granel · 39.660 kg · $10.211.404**, y al desplegarla verse los **10 pasos** (5 grupos × 2 tramos) arrancando por *16/03/2026→24/06/2026 · Ternero Recria (55 cab): 6.521 kg*. En **26/27** (foto al **29/08**, después de la segunda medición): Recría **Maíz 21.960 kg · $5.866.257** y **Concentrado Novillo 35 10 · 2.206 kg · $1.607.818**; Cría **Maíz 1.600 kg · $427.519** y **Concentrado 159 kg · $116.267**. ⚠️ **El que prueba el fix es el tramo del 16/03**: si esa línea no aparece, el respaldo de template volvió a quedar afuera. Sumando las dos campañas y las dos actividades tiene que dar **$18.745.481**, que es exactamente el costo del consumo medido ($17.021.396 de maíz + $1.724.085 de concentrado). ⚠️ **Los números de 26/27 se mueven con cada medición nueva** — los de 25/26 no, porque ese período ya está cerrado por mediciones | → [A-TEST-75](#a-test-75) `@productivo` |
...
| **A-FEAT-68** | 🔴 | Feat | **La ración medida NO es un número: es una CURVA que sube con el invierno — y la receta proyecta 2,2× de más** · Primera comparación medido vs proyectado, posible recién ahora que hay mediciones (pedida por el usuario 2026-08-29: *«habría que ver en función del consumo dado si hay divergencias contra lo proyectado hacia adelante»*). El resultado **no es que la receta esté mal — es que describe otra cosa**: `racion_pct_pv` es **un solo número** y el consumo real es **estacional**. Medido sobre el kilo-día del rodeo: **16/03→24/06 (100 d) = 0,43 %** · **24/06→24/07 = 1,35 %** · **24/07→24/08 = 1,51 %**. 🔑 **El último tramo pega casi exacto con la receta (1,50 %)**: el 1,5 % describe la **suplementación plena de invierno**, no el promedio del ciclo. En marzo-junio hay pasto y el suplemento es un cuarto de eso. 💸 **El costo de no arreglarlo**: proyectar 1,5 % todo el año sobre los mismos 9.092.495 kilo-día da **136.387 kg contra 62.710 reales** — **73.677 kg de más**, unos **$19,7 M** al precio de hoy. Y en un presupuesto a 2 años eso se repite. **Propuesta**: que `racion_pct_pv` admita **una serie por mes** (o por tramo), con el patrón *default del dato real, siempre editable* — arranca con lo medido y se puede pisar. ⚠️ **Lo que NO hay que hacer es ajustar el número al promedio (0,69 %)**: sería igual de falso, sólo que erra al revés. ⚠️⚠️ **CORRECCIÓN DEL USUARIO (2026-08-29), y cambia el enunciado**: *«el consumo es a discreción. Si comen más es porque están comiendo más. La estacionalidad no sabemos cómo afecta»*. 🔑 **Entonces `racion_pct_pv` no es un parámetro de manejo: es un RESULTADO observado.** Nadie fija la ración — se les da a discreción y el % es lo que salió de dividir. Eso tiene dos consecuencias: **(1)** la explicación estacional era **mi hipótesis, no un dato** — la serie sube, pero no está probado por qué; **(2)** para proyectar hay que usar **lo último medido** o una tendencia, **sin inventarle la causa**. Un modelo con estaciones que no están confirmadas es peor que uno que dice *«los últimos 30 días comieron a este ritmo»*. 📌 **Lo que sí queda firme**: un escalar único no sirve, porque el valor real se mueve entre 0,43 % y 2,31 % | → [A-FEAT-68](#a-feat-68) `@productivo` |
| **A-FEAT-69** | 🟡 | Feat | **La mezcla real es 97/3 y la receta dice 85/15** ([A-FEAT-68](#a-feat-68) es su hermana) · La receta de Recría reparte **Maíz 85 % / Concentrado 15 %** (`modo=pct_racion`, valor `0.85` y `0.15`). Lo medido en todo el ciclo es **maíz 60.860 kg (97 %) / concentrado 1.850 kg (3 %)**. 📌 **Pero la comparación honesta no es ésa**: el concentrado **recién arrancó el 22/07**, así que el promedio del ciclo lo diluye. En el último mes —el único donde conviven— la mezcla real es **~91/9**, todavía lejos del 85/15 pero mucho más cerca. 🔑 **Es el mismo problema que [A-FEAT-68](#a-feat-68)**: un número fijo para algo que cambia en el tiempo. Y tiene el mismo síntoma peligroso — **el total del costo no avisa**, porque los kilos se reparten igual; lo que queda mal es **cuánto de cada cosa hay que comprar**. ✅ **RESUELTO 2026-08-29 por el usuario — la mezcla es 90/10**: *«la ración es lo que está en el comedero ya mezclado 90/10»*. Y lo confirma el dato de la medición: **160 kg de concentrado en 1.600 de ración = 10 % exacto**. 🔑 **De dónde salió el 85/15**: es el % de inclusión del **Concentrado Terneros Recria** (*«desde los 100 kg, 15 % de inclusión»*), pero el que se está usando es el **Novillo 35 10**, que dice *«10 % de inclusión»* en su propia observación. La receta quedó con el número del otro concentrado. ⚠️ **Falta el cambio de dato**: poner `0.90` / `0.10` en las dos filas de insumo de Recría ([A-DAT-19](#a-dat-19)). Y **al volver al concentrado de terneros habrá que volver a 85/15** — o sea el % de inclusión debería salir del **insumo**, no de la receta | → [A-FEAT-69](#a-feat-69) `@productivo` |
| **A-DAT-19** | 🟡 | Dato | **Corregir la mezcla de la receta de Recría: 85/15 → 90/10** ([A-FEAT-69](#a-feat-69)) · En *Productivo → Actividades → Recría*, las filas **Maíz** y **Concentrado** tienen `modo=pct_racion` con `0.85` y `0.15`. Van a **`0.90`** y **`0.10`**, que es la mezcla real del comedero y el % de inclusión que declara el Concentrado Novillo 35 10. 📌 **No cambia ningún número medido** —el consumo real se mide, no se calcula con esto— pero sí **cuánto de cada cosa proyecta comprar** hacia adelante | → [A-DAT-19](#a-dat-19) `@productivo` |
| **A-BUG-91** | 🔴 | Bug | **HECHO 2026-08-29 — falta testear ([A-TEST-76](#a-test-76))** · **Un control en rojo permanente deja de ser un control** · Al cargar la entrega de 24.920 kg del 29/08, *«Lo comprado está explicado»* se puso en **rojo con todo perfectamente cargado**. No era un descuadre: `compradoTotal` incluía una entrega **posterior a la última medición**, y la identidad `consumo + cierre − apertura` sólo puede cubrir el período medido. ⚠️ **Y esto iba a pasar SIEMPRE**: cada vez que llega un camión entre dos mediciones —o sea la mayor parte del tiempo— el control quedaba en rojo. Fix: se compara contra **lo comprado dentro del período medido**, y lo que llegó después sale como **línea informativa aparte** (*«24.920 entregados el 29/08 o después: están en el stock y los va a imputar la próxima medición. No es un descuadre»*). 🔑 **La lección es la gemela de *el silencio miente*: el ruido también miente.** Un rojo que está prendido siempre entrena a ignorarlo, y el día que descuadre algo de verdad nadie lo va a mirar. Un control que no puede estar en verde **no es un control, es un adorno** | → [A-BUG-91](#a-bug-91) `@productivo` |
| A-TEST-76 | 🔴 | Test | **Los controles distinguen «falta medir» de «no cuadra»** ([A-BUG-91](#a-bug-91)) — en *Insumos → Stock → Maíz Granel → Mediciones*, con la entrega del **29/08 de 24.920 kg** cargada y la última medición del **29/08**: los tres controles de cantidad tienen que estar en **✓**, y aparte una línea que diga **«Entregado después de la última medición · 24.920»**. ⚠️ **El único ✗ legítimo hoy es «Lo entregado tiene respaldo: 24.920 de 91.580 sin respaldo»** — y tiene que desaparecer al vincularle la factura de Longo ([A-DAT-17](#a-dat-17)). Después, cargar una medición posterior y verificar que **la línea informativa desaparece** y esos kilos pasan a consumo o a remanente | → [A-TEST-76](#a-test-76) `@productivo` |
| **A-DAT-17** | 🔴 | Dato | **Falta vincular la factura de los 24.920 kg de maíz del 29/08 (LONGO)** · Es el **único control en rojo** que queda en la cadena, y es un rojo correcto: sin respaldo esos kilos **no tienen precio**, así que cuando la próxima medición los impute van a salir *«sin precio»* y **el tramo entero quedaría sin costo**. 📌 **Lo hace el usuario desde la app**: *Productivo → Insumos → Stock → Maíz Granel → Facturas* → la entrega del 29/08 → buscar la FC de Longo. 📌 Los 5 controles vuelven a verde en cuanto se vincule. ⚠️ **No urge para el número de hoy** —esos kilos todavía no se consumieron— pero sí **antes de la próxima medición**. ✅ **LA FACTURA YA ESTÁ (2026-08-31)**: el usuario la importó de ARCA — **FC 0002-00000531 del 30/08, neto $6.740.860**. 🔑 **Y cuadra exacto**: `$6.740.860 ÷ 24.920 kg = $270,50/kg` **redondo**, en línea con los anteriores ($267,50 y $267,05). Sólo falta hacer el vínculo. 📌 Este caso es el que originó [A-FEAT-73](#a-feat-73): la app tenía **todo** para proponerlo sola | → [A-DAT-17](#a-dat-17) `@productivo` |
| **A-FEAT-73** | 🔴 | Feat | **Que la app PROPONGA el vínculo cuando llega la factura de una entrega que la espera** — pedido del usuario 2026-08-31: *«acabo de importar la factura de maíz desde ARCA. ¿Debería la app proponer vincularla contra la compra de maíz que espera factura?»*. **Sí, y tiene todos los elementos.** En el caso real que lo disparó, el sistema sabía: hay una entrega **sin respaldo** (24.920 kg del 29/08, LONGO) y entra una FC de **LONGO** del 30/08 cuyo neto **dividido por esa cantidad da $270,50 redondo**, en línea con las anteriores. Tres señales independientes que apuntan al mismo lugar, y el usuario igual tuvo que ir a buscarla a mano. 🎯 **Es el norte administrativo aplicado** (`CLAUDE.md` § 🤖): no automatizar el vínculo —**proponer**, que el usuario confirme— porque un vínculo mal hecho **inventa stock o precio** y eso no se detecta después. 📌 **Cómo puntuar un candidato**: mismo **CUIT/proveedor** (fuerte) · **fecha cercana** a la entrega (±15 días) · **el precio derivado cae cerca de la mediana** de las otras entregas del insumo (la señal más valiosa, y ya existe el cálculo) · el respaldo está **libre o parcial** (`usoDeRespaldo()` ya lo sabe). 📌 **Dónde se muestra**: en *Insumos → Stock → Facturas*, como aviso arriba (*«hay 1 factura que podría corresponder a una entrega sin respaldo»*), y **también al importar de ARCA**, que es donde el usuario está mirando cuando pasa. ⚠️ **Nunca vincular solo, ni siquiera con las tres señales**: mostrar el candidato con el precio que resultaría, y que el vínculo lo haga una persona | → [A-FEAT-73](#a-feat-73) `@productivo` |
| **A-FEAT-70** | 🟡 | Feat | **Un tramo corto amplifica el error de medición — y la curva de ración lo va a heredar** ([A-FEAT-68](#a-feat-68) es su hermana) · El tramo **24/08→29/08** dura **5 días** y da **2,31 % del peso vivo**, cuando la serie venía **0,43 → 1,35 → 1,51**. ✅ **La medición estaba BIEN** — confirmado por el usuario 2026-08-29: *«la ración es lo que está en el comedero ya mezclado 90/10; luego maíz ya no quedaba en el silo de almacén y el concentrado se almacena en bolsas de 25 kg»*. O sea el silo estaba vacío y los 1.440 kg son todo lo que había. **Comieron más de verdad.** 🔑 **Lo importante no es este caso, es la propiedad**: un error de 500 kg mueve el tramo de 5 días un **11 %** y el de 100 días un **2 %**. Como el % de ración medido es lo que va a alimentar la proyección estacional de [A-FEAT-68](#a-feat-68), **un tramo corto ruidoso puede contaminar la curva entera**. **Propuesta**: al construir la curva, **ponderar por días** y **marcar los tramos de menos de ~10 días** como poco confiables — mostrarlos, no descartarlos en silencio. 📌 **Queda como propiedad a cuidar, no como sospecha sobre este dato** | → [A-FEAT-70](#a-feat-70) `@productivo` |
| **A-BUG-92** | 🔴 | Bug | **⚠️ NO ARREGLAR SIN CONFIRMAR [A-DEC-13](#a-dec-13) — mueve $349.050 de imputación** · **El «Resto sin lote» es INMORTAL: come todo el ciclo y nunca se muere** · Lo detectó el usuario mirando el reparto: *«volvemos al resto sin lote. ¿No debería estar o son las mortandades? Ojo de cómo lo tratamos porque puede generar errores»*. **Son las mortandades**, y la cuenta cierra exacto: **189 declaradas − 185 en lotes − 4 mortandades = 0**. Pero `grupoResto()` las mete como un grupo con **`hasta: null`** —sigue vivo para siempre— y encima las bajas se **prorratean sobre todos los grupos**, así que los lotes reales pierden cabezas que sí tienen. 💸 **Medido**: hoy el fantasma se lleva **$431.884**; los 4 muertos, comiendo sólo hasta el día que murieron, valen **$82.834**. **$349.050 mal imputados**, que salen de los 5 lotes reales. 🔑 **El total NO cambia** ($18.745.481 en los dos escenarios) — el kilo-día sólo reparte. Por eso **ningún control lo agarra**: es un error que no descuadra nada. ⚠️ **Y `conciliarCabezas()` recibe las bajas, las suma en `totalBajas`… y NO las usa para calcular la diferencia.** El dato para detectarlo estaba ahí sin usar | → [A-BUG-92](#a-bug-92) `@productivo` |
| **A-DEC-13** | 🔴 | Dec | **Cómo se trata la mortandad en el reparto del consumo** — hay que decidirla antes de tocar [A-BUG-92](#a-bug-92) · **(a)** *Cada muerto es un grupo con `hasta` = la fecha de su muerte*: come mientras vivió, deja de comer cuando murió, y **su costo queda identificado por animal y por fecha**. Es lo que pide la regla ya escrita (*«la mortandad es un costo y se adjudica; los muertos a su peso real»*) y da **$8.118 / $10.824 / $29.128 / $34.764** — se ve barato el que muere temprano y caro el que muere tarde, que es la verdad. **(b)** *El muerto no come nada*: su comida se reparte entre los vivos. Más simple, pero **esconde el costo de la mortandad** justo cuando se quiere medir. **(c)** *Como está hoy*: un grupo inmortal — **no es una opción, es el bug**. 📌 **RECOMIENDO (a)**, y ahora con el número que lo decide. El usuario puso la jerarquía: *«el objetivo es poder tener claridad como nº 1»*, y advirtió que lo que más duele es **tergiversar el ratio de consumo** (*«parecerá que el consumo fue superior… uno toma ratios incorrectos»*). 🔑 **Medido, el ratio casi no se mueve**: **A 0,715 % · B 0,716 % · C 0,721 %** del peso vivo. Entre el peor y el mejor hay **0,8 %** — nada que ver con el *«están comiendo 3 pero estaban comiendo 2,5»* que se temía (eso sería 20 %). Son 4 muertos sobre 189 y murieron temprano. **Eso libera la decisión: se puede elegir por CLARIDAD, que era el objetivo nº 1, sin pagar nada en el ratio.** Y (a) **no es compleja** — ya está calculada y verificada. ⚠️ **Antes hay que resolver un dato**: las 4 «sin lote» son **2 machos + 2 hembras**, pero los muertos son **3 machos + 1 hembra** — el total cierra y la composición no. Ver [A-DAT-18](#a-dat-18) | → [A-DEC-13](#a-dec-13) `@productivo` |
| **A-DAT-18** | 🔴 | Dato | **Tres fuentes dan tres números distintos de machos destetados: 106 / 107 / 104** · Apareció al investigar [A-BUG-92](#a-bug-92). **`ciclos_recria`** dice **106 machos + 83 hembras**; los **movimientos de hacienda** del destete (23/02) dicen **100 Ternero + 7 Torito = 107 machos + 82 hembras**; los **lotes** suman **104 machos + 81 hembras**. Los totales cierran (189 / 189 / 185+4) pero **la composición por sexo no**. 📌 Se suma una cuarta rareza: el movimiento del **26/04** dice `Torito −8 (cambio categ → Toro)` **cuando el destete había registrado 7**, y el lote de toritos dice **9**. 📌 La nota del ciclo ya avisaba: *«stock_ciclos 25/26 dice 107 machos destetados y la pesada tiene 103 — 4 sin pesar»*. ⚠️ **Hoy no rompe el número** —el consumo se reparte por kilo-día y el total no depende del sexo— **pero sí va a romper el margen por grupo** y cualquier control de existencias. ✅ **RESUELTO EN PARTE 2026-08-29 por el usuario**: *«ojo con los 8 toritos que van a toros, son de la campaña anterior y se van antes del consumo. El error real es de 7 vs 9. La app toma ok 9»*. Entonces: **(1)** el `Torito −8` del 26/04 es de la **campaña anterior** y no toca este rodeo — no hay que tocarlo; **(2)** **los toritos del ciclo son 9**, y el movimiento de destete que dice **7 está mal**. 🔑 **Con eso la composición cierra sola**: lotes vivos **104 M + 81 H** más los muertos **3 M + 1 H** = **107 machos + 82 hembras = 189**, que es exactamente lo que suman los movimientos de destete. Y dentro de los machos, el movimiento dice `Ternero 100 + Torito 7` cuando debería ser **`Ternero 98 + Torito 9`** — **dos terneros que en realidad eran toritos**, y por eso el total de machos nunca se movió. ⚠️ **Lo que queda mal es `ciclos_recria`: dice 106 M + 83 H y la composición real es 107 + 82.** El total (189) está bien en las dos fuentes, así que **el consumo no cambia** (el kilo-día sale de los lotes, no del ciclo) — pero **el valor de entrada sí**, porque macho y hembra pesan distinto. ❓ **Falta que el usuario confirme el 98**, que es deducción mía | → [A-DAT-18](#a-dat-18) `@productivo` |
| **A-FEAT-71** | 🔴 | Feat | **La PÉRDIDA POR MORTANDAD como número propio: $5.304.059 en 4 animales** · Pedido del usuario 2026-08-29: *«idealmente tener la pérdida por mortandad identificada en costo por el animal = kg de compra × precio de compra + costo de alimentación perdido, lógicamente a fecha de muerte»*. **Es exactamente lo que sale de [A-DEC-13](#a-dec-13) opción (a)**, sumándole el valor de entrada — y ya está calculado: **15/04 $1.320.636 · 25/04 $1.323.342 · 26/06 $1.312.798 · 02/07 $1.347.282**. 🔑 **El hallazgo que ordena las prioridades**: la **alimentación es sólo el 1,6 %** de la pérdida y **el animal el 98,4 %**. Encaja con lo que ya sabíamos (*el precio de entrada es el 93 % del resultado*). **Consecuencia**: para medir la mortandad, **la precisión de la comida casi no importa** — lo que importa es tener bien el valor de entrada. Eso simplifica el diseño y dice dónde poner el esfuerzo. 📌 **Dónde va**: es un renglón del margen de Recría, no un reporte aparte — el animal muerto **ya está pagado** y su costo tiene que verse. ⚠️ **Depende de [A-DAT-18](#a-dat-18)**: el valor de entrada usa el peso neto de macho o hembra, y la composición del ciclo está mal (106/83 en vez de 107/82) | → [A-FEAT-71](#a-feat-71) `@productivo` |
...
`entrega_factura` gana **`origen`** (`arca` | `template`) y `factura_id` pasa a ser una
**referencia genérica**. Sigue sin FK — que es justamente lo que permitió esto sin migrar nada.

| origen | apunta a | el neto es |
|---|---|---|
| `arca` | `msa\|pam\|ma.comprobantes_arca` | `imp_neto_gravado` |
| `template` | `public.cuotas_egresos_sin_factura` | ⚠️ **el `monto`** — un template no tiene IVA discriminado |

Se vincula **la CUOTA, no el template**: es la que tiene fecha y monto concretos.

📌 **Y el buscador se escribió una sola vez**: `traerRespaldos()` vive en el lib y lo usan las
**tres** pantallas que lo necesitan — el panel de Facturas, el modal de compra y el margen. Tres
consultas al mismo concepto es como terminan mostrando cosas distintas.

**El caso que lo motivó ya se puede cerrar**: la cuota del maíz del 16/03 está en la base
(*«1.740 Kg Maiz Castillo a 193.000 la ton (ya descontado 20 % por gorgojos)»*, **$335.800**,
conciliada), así que esa entrega deja de quedar *«sin respaldo»* para siempre.

---

---

## <a id="a-dat-13"></a>A-DAT-13 / A-DAT-14 — reparación de los vínculos, 2026-08-28

**Con OK explícito del usuario**, y **sin tocar la de Pereyra** porque él lo pidió así.

| Entrega | Qué tenía | Qué quedó | Control |
|---|---|---|---|
| **16/03** · 1.740 kg | `origen='arca'` apuntando a una **cuota** · sin precio | `template` · **$192,9885** | subtotal **$335.800** = el monto de la cuota ✅ |
| **06/05** · 7.300 kg | sin precio | **$262,0000** | subtotal **$1.912.600** = el neto ✅ |
| **24/06** · 20.100 kg | **$332,7114** (neto ÷ entregado) | **$267,50** (el de su factura) | subtotal **$5.376.750** · el neto es $6.687.500 → **quedan $1.310.750**, que es el **anticipo de 4,9 t** ⏳ |
| **17/06** · 4.960 kg | sin precio | **$238,3528** *(el usuario lo autorizó después)* | subtotal **$1.182.230** = el neto ✅ |

Se actualizó también `movimientos_insumos.costo_unitario` de esas tres, para que la entrega no
quede sin precio si algún día se le saca el vínculo.

⚠️ **La diferencia de $1.310.750 en Longo es correcta y tiene que estar**: es mercadería facturada
y todavía no imputada a ninguna entrega. Se apaga cuando se cargue la entrega del **24/07** y se
le vinculen los 4.900 kg de esa misma factura.

---

## <a id="a-doc-11"></a>A-DOC-11 — Un respaldo parcial no es una compra nueva

Al explicar el arreglo de Longo, el usuario preguntó:

> *"¿O sea que es hacer una compra nueva de 4900 para la factura de 25T?"*

**No.** Y la respuesta importa porque el error habría sido invisible:

| Si se hubiera hecho | Qué pasaba |
|---|---|
| Una **compra** nueva de 4.900 kg | el stock subía de **66.660 a 71.560** — mercadería que nunca llegó |
| Un **vínculo** nuevo de 4.900 kg | el stock **no se mueve**: sólo se declara quién respalda esa parte |

Las 4,9 t **ya estaban adentro** de la entrega de 25.000 kg del 24/07. Lo que faltaba era un
**segundo respaldo para esa misma entrega**.

### La analogía que lo cerró

> **Como pagar una compra con dos cheques: son dos comprobantes, una sola compra.**

### Por qué queda escrito

**Es la pregunta natural.** El modelo de muchos-a-muchos entre entregas y facturas no es obvio —
se diseñó así justamente porque la realidad no coincide— y equivocarse acá **inventa stock sin que
nada avise**. La próxima vez que alguien tenga la duda, ésta es la respuesta.
