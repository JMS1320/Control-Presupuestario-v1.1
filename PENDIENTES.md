...
| A-BUG-102 | 🔴 | **Alta** | **El mail de Detalle de pago anuncia un ECHEQ como «Transferencia»** — en `lib/pagos/medios-pago.ts` el rótulo del anticipo está fijo en `'Transferencia'` sin mirar `estado_pago`. El cuerpo además promete *«les llegará el comprobante de transferencia de Galicia»*: **el proveedor espera una acreditación que no existe**. Caso: IGLESIAS 04/09, $2.454.000 con `estado_pago='echeq'` | 3 mails encolados `@cashflow` |
| A-BUG-103 | 🔴 | **Alta** | **«Pasar a echeq» no crea el echeq**: marca `anticipos_proveedores.estado_pago='echeq'` pero **no inserta fila en `<schema>.cheques`**, y por eso **no pide número ni fecha de débito** (detectado por el usuario). Consecuencias: el desglose no lo puede nombrar, no hay fecha de cobro y **el Cash Flow no sabe cuándo debita**. Deja además `estado_pago='echeq'` contra `metodo_pago='transferencia'` en la misma fila | Caso IGLESIAS `@pagos` |
| A-BUG-104 | 🔴 | **Alta** | **El detalle de pago se manda aunque la resta NO cierre.** IGLESIAS: `3.554.000 − 2.454.000 − 57.400,40 = 1.042.599,60` pendiente y la factura sigue en `pagar`, pero el mail lista importes que no suman **y no muestra la diferencia**. Es la § 🧮 de `CLAUDE.md` (mismo caso que A-TEST-32). **Falta el control antes de encolar** | 3 mails encolados `@cashflow` |
| A-BUG-105 | 🟡 | Media | **Un anticipo aplicado a una factura se suma como si fuera otra factura.** Al encolar los dos juntos el mail dijo *«Importe facturas: $6.008.000»* = anticipo 2.454.000 + factura 3.554.000: **el anticipo se cuenta dos veces** (está aplicado *a* esa factura, `factura_id` cargado). `totalBruto` suma `imp_total` de todos los items sin distinguir | 3er mail encolado `@cashflow` |
