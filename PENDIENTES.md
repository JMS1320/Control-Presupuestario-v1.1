...
| A-FEAT-95 | 🔴 | **Alta** | **Guardar la documentación de la venta en Drive** (liquidación + romaneo), **en el mismo árbol que las facturas de los subdiarios** — pedido del usuario. Reusar `archivarEnDrive` del GAS (find-or-create, subcarpetas por empresa/campaña/mes) en vez de inventar un almacenamiento nuevo. Guardar el `file_id` en `romaneos` para poder abrirlo con 👁 desde la venta, como ya se hace con las FC | `@ganaderia` |
| A-FEAT-96 | 🔴 | **Alta** | **RINDE Y DESBASTE POR GRUPO DE PRECIO** — *«el dato más importante para nosotros»* (usuario). Un grupo = un precio, aunque la categoría sea la misma: vaca y toro al mismo precio siguen siendo **2 grupos**, y dentro de vaca puede haber 2-3 precios = 2-3 grupos. 🔴 **El romaneo NO lo puede dar solo**: su columna *Vivo* no es una pesada — el frigorífico reparte el total **usando el rinde global**, y por eso los 9 grupos dan **53,58 % todos** (verificado). El rinde real sale de cruzar **nuestras pesadas** con el kilo de carne del garrón | `@ganaderia` |
...
| A-FEAT-98 | 🔴 | **Alta** | **El FLETE / CZ se carga desde la VENTA y genera el compromiso en Cash Flow.** Como no llega factura, la vía es el **anticipo** — pero **no se crea desde anticipos**: se crea y se edita **desde la venta**, que es la que tiene los campos de CZ para dar el total. La fila del anticipo queda en su tabla. Setup del flete: **arranque de km, camino elegido, editable**. Caso vivo: el flete de Arrebeef **ya está acordado** y falta cargarlo | `@ganaderia` `@cashflow` |
| A-FEAT-99 | 🟡 | Media | **Comparar el negocio contra los otros canales**: qué hubiera sido vender a **Cañuelas**, a **matarife zonal** u otro canal, contra lo que fue Arrebeef. ⚠️ **Ya existe base**: `resolverPrecioHacienda` / `categoriaPrecio` y el módulo de comercialización (CZ = comisión + flete + otros, todo a $/kg vivo) — **buscar antes de escribir**. Lo nuevo es contrastar contra una venta **real ya liquidada** | `@ganaderia` |
...
| A-TEST-90 | 🔴 | Test | **Probar el parser de romaneo** (`A-FEAT-94`) cuando exista la pantalla. Caso base: el romaneo de ARRE BEEF del 04/09 tiene que dar **$18.750.900**, 3.354 kg gancho, 6.260 vivos, 10 cabezas y rinde 53,58 — y avisar que faltan 2 medias reses. **Adversarios**: (1) un romaneo de **2 hojas** — las filas no se deben mezclar entre páginas; (2) un PDF que **no sea** un romaneo → debe proponer vacío con avisos, **nunca romper**; (3) editar un valor propuesto y confirmar → se guarda el editado, no el del PDF | ← [A-FEAT-94](#a-feat-94) `@ganaderia` |
