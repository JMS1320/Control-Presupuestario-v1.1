...
| A-OP-10 | 🔴 | Baja | **`scripts/proximo-id.mts` — que el ID libre lo diga una orden, no la memoria** (propuesto 2026-08-27, a raíz del choque A-BUG-54). Hoy el chequeo previo es un loop de bash escrito en el tablero: **depende de que alguien lo corra**, y el `verificar-parser-pendientes.mts` sólo detecta el duplicado **después** de que ya se propagó a varios archivos. Un script que imprima el próximo libre de cada familia convierte la prevención en un comando de 2 segundos. ⚠️ **No elimina el choque**: dos terminales que lo corren al mismo tiempo reciben el mismo número. Lo que hace es sacar del medio el modo de falla real —*no mirar*— y el `sort -n` mal hecho. La red sigue siendo el verificador | → [A-OP-10](#a-op-10) `@general` |
...
| **A-BUG-63** | 🟡 | Bug | **HECHO 2026-08-27 — falta testear ([A-TEST-52](#a-test-52))** · **El costo MEDIDO se perdía si ninguna receta lo reclamaba** — el margen sólo mostraba el consumo medido cuando encontraba una fila de `actividad_insumos` con nombre parecido. Al nombrar los productos por su **formulación comercial** (`Concentrado Novillo 35 10`) y no por actividad, el match por texto contra la receta (`Concentrado` a secas) **deja de funcionar — y fallaba en silencio**: el costo no aparecía en ningún lado y nada avisaba. Fix: **el consumo medido es un hecho y entra al margen por su cuenta**, con el nombre del producto y la aclaración *«sin fila de receta que lo proyecte»*. La receta sirve para proyectar hacia adelante; el vínculo entre las dos es **comodidad de presentación, no requisito** | → [A-BUG-63](#a-bug-63) `@presupuesto @productivo` |
...
| A-TEST-52 | 🔴 | Test | **El costo medido aparece aunque no haya receta** ([A-BUG-63](#a-bug-63)) — con mediciones cargadas de `Concentrado Novillo 35 10`, en *Presupuesto → Margen → Recría* tiene que aparecer una fila **con ese nombre** y su monto, diciendo *«sin fila de receta que lo proyecte»*. Verificar que **no desaparece** al no haber ningún `actividad_insumos` que se llame igual, y que **no se duplica** con la fila de receta cuando sí matchean (caso `Maíz Granel` con el `producto` cargado) | → [A-TEST-52](#a-test-52) `@presupuesto @productivo` |
...
## <a id="a-bug-63"></a>A-BUG-63 — El costo medido se perdía si ninguna receta lo reclamaba
