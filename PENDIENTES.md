...
- **PENDIENTE (avanzar con cuidado, el usuario pidió análisis primero)**: que el **export de la app genere el TXT con punto** en vez de coma (Importe comprobante / Base de cálculo / Importe de la retención). Evaluar: (a) cambiar el separador en el generador del TXT; (b) NO tocar los otros campos (fechas usan `/`, no tienen coma); (c) confirmar que Excel/PDF del mismo export **no** deben cambiar (esos son es-AR con coma, solo el TXT va a ARCA); (d) idealmente separador configurable o al menos claramente aislado. Código del generador: buscar en `components/vista-facturas-arca.tsx` (modal "Generar Export"). Detalle: KNOWLEDGE (#sicore #arca) + `memory/project_sicore_error_importacion_arca`.
