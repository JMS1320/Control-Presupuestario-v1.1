...
| **A-BUG-77** | 🟡 | Bug | **La compra de insumos usa `type="number"` para cantidad y costo, contra la convención del proyecto** — `CLAUDE.md` § *Convención Inputs Monetarios (es-AR)* dice **`type="text"` siempre, nunca `type="number"` para montos**, y el modal *Nueva Compra de Insumos* (`vista-sector-productivo.tsx`) los tiene los dos como `number`. Consecuencia práctica: hay que escribir **`238.35` con punto**, no `238,35` — al revés de todo el resto de la app. Con precios de maíz de 4 y 5 cifras y decimales es fácil equivocarse y que quede **un precio mil veces mayor** sin que nada avise | → [A-BUG-77](#a-bug-77) `@productivo` |
