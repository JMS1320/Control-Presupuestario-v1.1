...
- **⭐ B-FEAT-RENOVAR-CAMPAÑA — Replicar templates/cuotas para la nueva campaña (2026-07-03, URGENTE)**: la campaña 2025/2026 cerró el 30/6. Muchos templates (sobre todo `tipo_template='fijo'`: seguros, impuestos mensuales, cargas sociales, IIBB, SICORE) tienen su **última cuota en jun/jul 2026** y hay que **generar las cuotas de la campaña nueva** (2026/2027) o el usuario pierde de vista los vencimientos. **HOY NO EXISTE** un sistema para esto (verificado en código + PENDIENTES). **Propuesta:** feature "Renovar campaña" — seleccionar templates → generar el próximo lote de cuotas copiando el cronograma/importes del período anterior (fechas corridas al nuevo período), con revisión antes de confirmar. Los `abierto` (comisiones bancarias, Caja, etc.) se cargan solos vía conciliación/manual, no necesitan renovación programada. **Interino:** se pueden generar las cuotas de los `fijo` por SQL en bloque si el usuario da el cronograma (importes + frecuencia 2026/2027).
  ...
  - **📋 GENERADOR DE REGENERACIÓN — diseño acordado (2026-07-18):**
    ...
    - **🔴 A RETOMAR — contexto fundamental (2026-07-18, preguntas del usuario + respuestas Claude; ver memoria [[project_generador_renovacion_templates]]):**
      1. **Deseleccionar previstas (A CONSTRUIR):** falta un **checkbox por fila** en "Previstas a generar" para **excluir de ESTA corrida** (temporal, **distinto** de destildar `aplica_generacion` persistente). Hoy todas las previstas se generan sí o sí.
      2. **Template que NO genera igual debe existir en la campaña nueva (A DECIDIR + INVESTIGAR):** un abierto (Caja/comisiones) debería tener su fila del período nuevo **sin cuotas** (para Pago Manual/conciliación). Abierto: ¿**fila nueva vacía** (Modelo A → desactivar la vieja para no duplicar) o la **fila existente persiste** entre períodos? → **investigar cómo se comportan hoy los abiertos al cruzar de año** (¿la conciliación matchea por template sin mirar `año`?). Si persisten solas, no crear nada.
      3. **Fechas visibles (A DECIDIR UI):** hoy la matriz muestra **solo el monto** + mes/año en encabezado; el **día real** (preservado, usado al generar) **NO se muestra** → podría mostrarse (tooltip/subtexto).
      4. **Varias cuotas por mes (A DECIDIR):** confirmado que pasa (**Seguro Accidentes**: 3 meses con 2 cuotas). Matriz 1-celda/mes NO las representa. Idea usuario: **mostrar la SUMA + aviso** (badge "N cuotas"); definir si al generar se crean **separadas o una sola sumada**.
      5. **UATRE datos viejos:** cargado tipo anual (15 cuotas ene-2025→jun-2026, con ceros) → al correr +1 año solapa 2026-H1 y propone ceros. **Mitigado con Vaciar fila.** Mejora de fondo: pre-cargar solo desde un rango limpio de 12 meses.
      6. **Replicar a meses puntuales (A MEJORAR):** hoy replica a los 12 base; falta elegir meses específicos.
      7. **Otros detalles ya listados:** `fecha_vencimiento` no se setea (solo `fecha_estimada`) · fila vieja NO se desactiva · master reusado del origen · Acciones cae en 2028 por shift +2 (ver 25/26 intermedia = consulta contable) · Interbancarias opt-in.
