...
| A-TEST-91 | 🔴 | Test | **Probar el flete de la carga** (`A-FEAT-98`). Productivo → Hacienda → 🚚 Flete → carga del 03/09 (Arre Beef). (1) Cargar camino, km, arranque y $/km → el total se calcula solo; (2) escribir un total a mano → **manda el escrito** y se ve que difiere del calculado; (3) poner el transportista y guardar → aparece el compromiso en **Cash Flow** con esa fecha; (4) verificar que el transportista quedó en `proveedores`. **Adversarios**: km del viaje **menores al arranque** → debe cobrar el arranque y decirlo; **volver a guardar** → NO puede quedar un segundo compromiso | ← [A-FEAT-98](#a-feat-98) `@cashflow` |
