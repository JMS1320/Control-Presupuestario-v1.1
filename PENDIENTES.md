...
**✅ Hecho 2026-09-21** (commit `A-SEC-06` en `desarrollo`): nuevo `lib/auth/guard-sesion.ts` con
`exigirSesion()` — `getUser()` (valida el JWT) + exige `app_metadata.role` (una cuenta sin rol
recibe 403, igual que la app la manda a `/no-access`) — y `respuestaSinAcceso()` (401/403 JSON,
nunca redirect). Inyectado como **primera línea** de los 41 handlers de las 35 routes que no tenían
guard. Se verificó que **todas** se llaman desde el navegador con cookie (GAS no llama de vuelta a
la API), así que ninguna necesita token compartido. `/api/admin/*` sigue con `exigirAdmin()` y
`/api/perfil/avatar` ya validaba. `tsc`: 113 errores antes y después.

**🟡 Falta testear** ([A-TEST-81](#a-sec-03)): con sesión → todo igual; sin cookie →
`401 {ok:false}`; con cuenta sin rol → `403`. Y que ninguna pantalla haya quedado esperando otro
formato de error.
