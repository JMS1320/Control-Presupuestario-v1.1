...
| A-FEAT-105 | 🟡 | Media | **Botón 🧪 Probar en Principal — nivel 1: lógica pura, CERO escritura.** Corre las funciones con datos fijos (los reales del 03/09) y compara contra los números esperados. 🔑 **Corre en el navegador del usuario**, que es lo único que ve las diferencias de entorno: el parser que andaba en Node y devolvía cero en Chrome habría salido ahí. Los **cuatro bugs del 06/09 los agarra este nivel** y ninguno necesitaba escribir. ⏳ Nivel 2 (clicks visibles, sin confirmar) después; nivel 3 (escribir) sólo con `A-DEC-18` resuelto | `@testing` |
