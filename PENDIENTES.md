...
| A-FEAT-94 | 🔴 | **Alta** | **Cargar el ROMANEO y cerrar la venta a la res.** Analizado del real → `MODULO_HACIENDA` § 19. **3 tablas**: `romaneos` (cabecera: frigorífico, tropa, guía, DTA, faena, kg vivos/gancho, rinde, total) · `romaneo_medias` (20 filas, **2 por animal** — el garrón se repite y NO es duplicado) · `romaneo_lineas` (9, la liquidación agrupada). Cuelga de **`productivo.cargas`**, que ya existe: un camión, un romaneo, N ventas adentro. Completa `stock_ventas.kg_carne` (vacas 1.748 / toros 1.606) y con eso **el importe se calcula solo** — cierra el hueco que § 18.3 dejó abierto a propósito | Romaneo ARRE BEEF 04/09 `@ganaderia` |
