...
| B-BUG-PDF-DETALLE | 🔴 | Media | **PDF "Detalle de pago" no muestra descuentos / SICORE** (2026-07-21). Al descargar el detalle de pago (`lib/pagos/pdf-detalle-pago.ts`), no aparecen el **descuento aplicado** ni la **retención SICORE** (ni el desglose transferencia/retención). Debería mostrar: importe factura, descuento pronto pago, retención SICORE (con quincena/cert), y el neto efectivamente pagado. Revisar qué campos recibe el generador y de dónde los toma (`descuento_aplicado`, `monto_sicore`, `sicore` de `comprobantes_arca` / `sicore_retenciones`). |
| B-FEAT-PAGO-MULTIMEDIO | 🔴 | Media | **Detalle de pago con VARIOS medios (transferencia + echeq)** (2026-07-21, NO desarrollado). Caso Longo: una FC se paga con **anticipo por transferencia** + **echeq por el saldo** (neto de retención). Hoy el detalle de pago no contempla un pago **multi-medio** → debería mostrar los dos tramos: la transferencia (anticipo) y el echeq (con banco/nro/fecha de cobro), + la retención SICORE, sumando al total de la factura. Requiere: (a) modelar/leer que un pago puede tener N medios (anticipo `anticipos_proveedores` + cheque `msa.cheques` + transferencia extracto), (b) reflejarlo en el PDF/pantalla de detalle. Diseñar. Relación con [[project_sicore_cash_flow_estado]] (echeq) y la vinculación de anticipos. |
