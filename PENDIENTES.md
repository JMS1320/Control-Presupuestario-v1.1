...
| B-FEAT-17 | 🔴 | Media | **Precios de mercado desde web (entresurcosycorralesya.com)** — traer Prom.Kilo / Kilo+ / Kilo− / Bulto por categoría-rango (URL parametrizable `?desde=&hasta=`) para poblar los precios del análisis de engorde según nuestros kilajes/categorías. **La tabla se carga por JS** (no viene en el HTML) → NO sirve fetch simple. Plan: (1) encontrar la **API JSON** que llama la página (F12 Network) → endpoint que la consulta; (2) mapear categorías del mercado ↔ nuestros segmentos/pesos + decidir qué precio/interpolación (matemática a discutir). Fallback: carga manual (ya existe). Estructura de la web cambia muy poco. (2026-07-09) |
