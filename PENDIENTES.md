...
- **B-FEAT-OTROS-INGRESOS — Template de ingresos sin factura/liquidación (símil "Otros Gastos") (2026-07-03)**: para registrar **cobros de ventas que solo tienen contrato** (no liquidación ni factura ARCA) — se imputan como ingreso a "Otros Ingresos" y fluyen por Cash Flow + conciliación, igual que "Otros Gastos" pero del lado del ingreso.
  ...
  - **⚠️ LIMITACIÓN a resolver:** el wizard **fuerza `es_bidireccional=false` cuando `es_multi_cuenta=true`** (`wizard-templates-egresos` l.293). O sea, NO se puede tener **sub-cuentas + ingreso** a la vez. "Otros Gastos" es multi_cuenta (por eso tiene sub-cuentas). Un "Otros Ingresos" con sub-cuentas **hoy no se puede** directo → hay que: (a) permitir multi_cuenta + bidireccional, o (b) que "Otros Ingresos" sea bidireccional sin multi_cuenta y que la cuota igual acepte `categ` (sub-cuenta) — hoy el selector de sub-categoría solo aparece si `es_multi_cuenta`.
