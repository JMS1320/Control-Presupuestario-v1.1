...
- **⭐ B-FEAT-RENOVAR-CAMPAÑA — Replicar templates/cuotas para la nueva campaña (2026-07-03, URGENTE)**: la campaña 2025/2026 cerró el 30/6. Muchos templates (sobre todo `tipo_template='fijo'`: seguros, impuestos mensuales, cargas sociales, IIBB, SICORE) tienen su **última cuota en jun/jul 2026** y hay que **generar las cuotas de la campaña nueva** (2026/2027) o el usuario pierde de vista los vencimientos. **HOY NO EXISTE** un sistema para esto (verificado en código + PENDIENTES). **Propuesta:** feature "Renovar campaña" — seleccionar templates → generar el próximo lote de cuotas copiando el cronograma/importes del período anterior (fechas corridas al nuevo período), con revisión antes de confirmar. Los `abierto` (comisiones bancarias, Caja, etc.) se cargan solos vía conciliación/manual, no necesitan renovación programada. **Interino:** se pueden generar las cuotas de los `fijo` por SQL en bloque si el usuario da el cronograma (importes + frecuencia 2026/2027).
  ...
  - **📋 GENERADOR DE REGENERACIÓN — diseño acordado (2026-07-18):**
    ...
    - **🔴 REGISTRO DE LOS 7 PUNTOS (2026-07-18/19, preguntas usuario + respuestas + estado; ver memoria [[project_generador_renovacion_templates]]):**
      ...
      2. **Template que NO genera igual presente en la campaña nueva — ✅ RESUELTO (investigado):** el selector de abiertos (Pago Manual, `vista-cash-flow.tsx:1260`) filtra por `tipo_template='abierto'`+`activo=true`, **NO por `año`** → los abiertos **persisten entre años solos**, no hay que renovarlos. **Pregunta abierta menor del usuario:** si deselecciona un FIJO previsto, ¿la app debería ofrecer crear el template sin cuotas? Conclusión: un fijo vacío no sirve (no está en el picker de Pago Manual) → **deselección = simplemente no se crea**; si lo quiere después, re-corre el generador. (Sin acción.)
      ...
      4. **Varias cuotas por mes — A CONSTRUIR (acordado):** Seguro Accidentes tiene 3 meses con 2 cuotas. Matriz 1-celda/mes. Plan: **mostrar suma + marca de color + tooltip** con la composición; **generar = 1 cuota por celda** (como esté en la planilla); para casos raros, un **botón "detalle" por fila** (mini-editor propio, NO reusar el wizard). Pendiente de construir.
      5. **UATRE — la causa es dato viejo mal cargado (no del generador):** el arreglo de fondo es **borrar las cuotas en cero del ORIGEN 25/26 que nunca debieron estar** (dato — pedir OK al usuario e identificar cuáles). Mitigado ya con Vaciar fila + la advertencia (punto 7).
      6. **Replicar a meses puntuales — A MEJORAR:** hoy Replicar llena los 12 meses base; falta poder elegir meses específicos. (Aclaración: NO es un límite de generación — el generador ya genera todas las celdas llenas, incluidas las extra fuera de los 12.)
