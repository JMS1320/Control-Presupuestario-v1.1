...
| A-TEST-90 | 🔴 | **Permisos de roles editables** (A-FEAT-82) — **primero correr `scripts/60`**; hasta entonces la pantalla avisa y no guarda. Después: que el aviso ámbar **desaparezca** · editar los permisos de `contable`, guardar, y que **al recargar** el menú de esa cuenta muestre exactamente eso · ⚠️ **los tres candados de `admin`**: que la UI no ofrezca editarlo, que un `PATCH` a mano al endpoint devuelva 403, y que un `UPDATE` directo en la BD lo rechace el trigger — **son tres puertas distintas y hay que probar las tres** · que un `contable` con Sueldos quitado **no** pueda entrar por `/?seccion=sueldos` escribiéndolo a mano · dejar un rol **sin ninguna sección** y ver qué pasa al entrar con esa cuenta · y que la matriz de Permisos refleje lo editado | → [A-FEAT-82](#a-feat-82) `@general` |
