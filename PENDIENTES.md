...
| A-TEST-87 | 🔴 | **Subir foto de perfil** (A-FEAT-79) — **primero correr `scripts/59`**, que es lo único que falta. Después: subir un JPG y un PNG y ver que aparezcan en el avatar de la barra · **cambiar la foto dos veces seguidas** y verificar que se ve la nueva y no la vieja (es lo que el `?v=` tiene que resolver) · elegir **el mismo archivo dos veces** (el input se limpia a propósito para que el 2º intento dispare) · que un PDF y una imagen de más de 2 MB sean rechazados con su motivo — ✅ **estas 3 validaciones ya se probaron contra el endpoint (400/415/413)**, falta desde la pantalla · que un `contable` también pueda subir la suya · y el caso feo: **que la foto de un usuario no se pise con la de otro** | → [A-FEAT-79](#a-feat-79) `@general` |
