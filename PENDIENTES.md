...
- **B-FEAT-UNIFICAR-PORTAL — Unificar la config de "portal"/proveedor (2026-06-29)**: hoy "portal" está partido en 3 lugares sin sincronizar → `reglas_ctas_import_arca.estado` (credito, al importar) · `comprobantes_arca.fc` (Portal a mano en imputación) · `proveedores.fc_modo` (solo el buscador). Mejora: agregar campo **`fc`** a `reglas_ctas_import_arca` y que el import lo aplique (`buscarReglaCuit` lee solo cuenta+estado; el import hardcodea `fc='Buscar'`). Cierra el caso "NO Mail que debía ser Portal" (Autopistas). También: 2 columnas de mail en `proveedores` (`email_facturacion` 3 cargados = Config PDFs vs `email_pagos` 26 = Galicia) → aclarar/unificar. + Selector de `fc` manual por factura (no existe). Detalle en memoria [[config-proveedores-fragmentada]].
