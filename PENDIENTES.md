...
- **B-FEAT-VENTAS-COBROS — Importar ventas y registrar cobros (incl. retenciones que no entran a banco)** — *analizado 2026-07-01, falta definir 4 puntos*:
  ...
  - **AVANCE 2026-07-01**: ✅ BASE hecha (tabla `retenciones_recibidas` + `estado`/`fecha_cobro_estimada` en `comprobantes_venta`, ver RECONSTRUCCION). ⏳ Falta: import fac venta (ARCA Emitidos + Excel, con DEBUG), origen VENTA en cash flow, rama VENTA en el motor, UI retenciones + vínculo por CUIT, indicador ventas pendientes.
